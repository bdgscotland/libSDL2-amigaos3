---
title: sound.datatype/sound.datatype
manual: autodocs-3.5
...
```c
    ...
    METHODS
        ...
        GM_RENDER -- [Cause](autodocs-3.5/exec-library-cause-2.md) the graphic to render.  Currently the graphic
        ...
        DTM_COPY -- Copy the entire sound to the clipboard as 8SVX.
            ...
            NOTE: Up to and including V40 sound.datatype never stored
                  a valid [VoiceHeader](autodocs-3.5/include-datatypes-soundclass-h.md) with the file. This was fixed in V44.

                  Subclasses which support streaming data access may not
                  support this method.

        DTM_WRITE -- [Write](autodocs-3.5/dos-library-write-2.md) the entire sound to a file as 8SVX.
    ...
    TAGS
        SDTA_VoiceHeader (struct [VoiceHeader](autodocs-3.5/include-datatypes-soundclass-h.md) *) -- Set and get the base
            information for the sound.  [VoiceHeader](autodocs-3.5/include-datatypes-soundclass-h.md) is defined in
            [<datatypes/soundclass.h>](autodocs-3.5/include-datatypes-soundclass-h.md).

            NOTE: Up to and including V40 sound.datatype never returned
                  a valid [VoiceHeader](autodocs-3.5/include-datatypes-soundclass-h.md) for OM_GET and the [VoiceHeader](autodocs-3.5/include-datatypes-soundclass-h.md) data
        ...
        SDTA_SignalTask (struct [Task](autodocs-3.5/include-exec-tasks-h.md) *) -- [Task](autodocs-3.5/include-exec-tasks-h.md) to signal when the
            is complete, or if SDTA_Continuous is TRUE, when
            the next buffer is needed.

            Default for this tag is NULL. Applicability is (IS).

        SDTA_SignalBit (ULONG) -- [Signal](autodocs-3.5/exec-library-signal-2.md) mask to use with SDTA_SignalTask
            or 0 to disable.
        ...
        SDTA_SignalBitMask (ULONG) -- [Signal](autodocs-3.5/exec-library-signal-2.md) mask to use with SDTA_SignalTask
            or 0 to disable. This tag is an alias for SDTA_SignalBit.

            Default for this tag is 0. Applicability is (IS).

        SDTA_SignalBitNumber (BYTE) -- [Signal](autodocs-3.5/exec-library-signal-2.md) bit to use with SDTA_SignalTask
            or -1 to disable.
        ...
        SDTA_ReplayPeriod (struct [timeval](autodocs-3.5/include-devices-timer-h.md) *) -- Get the replay period,
            i.e. the time it takes for the complete sound to be played.
            If the sample size has not been set yet, the [timeval](autodocs-3.5/include-devices-timer-h.md) tv_sec
            and tv_micro members will be set to 0. If the sample is to
            be played continuously, both [timeval](autodocs-3.5/include-devices-timer-h.md) members will be set
            to 0xFFFFFFFF.
        ...
        SDTA_FreeSampleData (BOOL) -- This tag controls whether
            sound.datatype will call [FreeVec()](autodocs-3.5/exec-library-freevec-2.md) on the sample data
...
```c
        ...
        SDTA_SyncSampleChange (BOOL) -- If SDTA_Continuous was set to TRUE,
            ...
            progress, the [Task](autodocs-3.5/include-exec-tasks-h.md) to change the sample data will be made to
