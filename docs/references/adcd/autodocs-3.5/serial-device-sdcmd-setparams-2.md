...
```c
    ...
    2. The EOFMODE and QUEUEDBRK bits of io_SerFlags can be set/reset
       ...
       and 7WIRE bits of io_SerFlags can be used in [OpenDevice](autodocs-3.5/serial-device-opendevice-2.md) calls.
...
```
  			NOTE that the following fields are filled in by [Open](autodocs-3.5/dos-library-open-2.md)
```c
...
```c
       ...
       io_ExtFlags     additional serial flags (bitdefs in [devices/serial.h)](autodocs-3.5/include-devices-serial-h.md)
```
...
```c
       ...
       io_SerFlags     see [devices/serial.h](autodocs-3.5/include-devices-serial-h.md) for bit equates, NOTE that x00
...
```
   ...
   SEE ALSO
	[exec/OpenDevice()](autodocs-3.5/serial-device-opendevice-2.md)
