...
section: printer-device-cmd-write-2
functions: []
libraries: []
---
...
```c
      ...
      not assume that the MARGINS or TABS can be set.  [Close](autodocs-3.5/dos-library-close-2.md) to half the
...
```c
       printer.h, parallel.device, serial.device, [Preferences](autodocs-3.5/include-intuition-preferences-h.md)
```
