...
```c
      ...
      After AbortIO(), you must generally do a [WaitIO()](autodocs-3.5/exec-library-waitio-2.md).
```
   INPUTS

```c
      iORequest  -- pointer to the [IORequest](autodocs-3.5/include-exec-io-h.md) Block that is to be aborted.
```
   RESULTS
