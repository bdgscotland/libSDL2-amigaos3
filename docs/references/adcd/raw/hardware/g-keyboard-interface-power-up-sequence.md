...
test.  Whenever the keyboard is powered up (or  [restarted](../Hardware_Manual_guide/node0178.html)  -- see below), it
must not transmit anything until it has achieved synchronization with the
computer. The way it does this is by slowly  [clocking out 1 bits](../Hardware_Manual_guide/node0176.html#line8) , as
...
be in any state imaginable but must not be adversely affected by the
 [garbage character](../Hardware_Manual_guide/node0176.html#line14)  it will receive.  Again, because it receives a key
...
uses raw keycodes.
   ...
   Warning:
   --------
   The keyboard must not transmit a " [lost sync](../Hardware_Manual_guide/node0176.html) " code after
   re-synchronizing due to a power-up or  [restart](../Hardware_Manual_guide/node0178.html) ; only after
