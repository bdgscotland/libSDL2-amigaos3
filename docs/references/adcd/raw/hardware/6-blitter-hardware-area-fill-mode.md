...
only one bit set per horizontal row.  A special  [line draw mode](../Hardware_Manual_guide/node0128.html)  is
available for this operation.  Use a standard copy blit (or any other
blit, as area fills take place after all  [shifts](../Hardware_Manual_guide/node011F.html) ,  [masks](../Hardware_Manual_guide/node011F.html#line42)  and logical
combination of sources).   [Descending mode](../Hardware_Manual_guide/node0120.html)  must be used. Set either the
inclusive-fill-enable bit (FILL_OR, or bit 3) or the exclusive-fill-enable
bit (FILL_XOR, or bit 4) in  [BLTCON1](../Hardware_Manual_guide/node001A.html) . The inclusive fill mode fills
...
There is another bit (FILL_CARRYIN or bit 3 in  [BLTCON1](../Hardware_Manual_guide/node001A.html) ) that forces the
