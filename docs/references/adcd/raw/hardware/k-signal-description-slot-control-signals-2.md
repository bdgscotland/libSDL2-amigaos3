...
Slave (/SLAVEn)
   ...
   and the bus controller asserts  [/BERR](../Hardware_Manual_guide/node02B1.html#line4) .  The bus controller will assert
   /SLAVEn back to the interrupting device selected during a
```c
    [Quick Interrupt cycle](../Hardware_Manual_guide/node02BA.html) , so any device supporting  [Quick Interrupts](../Hardware_Manual_guide/node02AC.html) 
```
...
Configuration Chain (/CFGINn, /CFGOUTn)
   ...
   During the  [autoconfiguration](../Hardware_Manual_guide/node02C7.html)  process, an unconfigured Zorro III PIC
