...
around and combining them with logic operations.  This is sufficient for
 [moving graphic images](../Hardware_Manual_guide/node011D.html#line46)  around, so long as the images stay in the same
...
shifted left or right by a few pixels. To this end, both the A and B
 [DMA channels](../Hardware_Manual_guide/node011B.html)  have a barrel shifter that can shift an image between 0 and
15 bits.
...
The shift value for the A channel is set with bits 15 through 12 of
 [BLTCON0](../Hardware_Manual_guide/node001A.html) ; the B shift value is set with bits 15 through 12 of  [BLTCON1](../Hardware_Manual_guide/node001A.html) .
For most operations, the same value will be used for both shifts.  For
shifts of greater than fifteen bits, load the  [address register pointer](../Hardware_Manual_guide/node011B.html#line6) 
of the destination with a higher address; a shift of 100 bits would
require the  [destination pointer](../Hardware_Manual_guide/node011B.html#line6)  to be advanced 100/16 or 6 words (12
...
ability to mask the first and last word of each row coming through the A
 [DMA channel](../Hardware_Manual_guide/node011B.html) .
...
```
To do this, we point the B  [DMA channel](../Hardware_Manual_guide/node011B.html)  at the bitmap containing the
source image, and the D  [DMA channel](../Hardware_Manual_guide/node011B.html)  at the screen bitmap. We use a shift
value of 5.  We also point the C  [DMA channel](../Hardware_Manual_guide/node011B.html)  at the screen bitmap. We
...
last four bits (2 times 16, less 23, less 5) of the last word alone. The A
 [DMA channel](../Hardware_Manual_guide/node011B.html)  comes to the rescue.  We preload the A  [data register](../Hardware_Manual_guide/node011B.html#line31)  with
...
four bits set to zero ($FFF0).  We do not enable the A  [DMA channel](../Hardware_Manual_guide/node011B.html) , but
...
the A channel is 1 (for a  [minterm](../Hardware_Manual_guide/node011D.html)  of AB) and pass along the original
destination data (from the C channel) wherever A is 0
                    _
(for a  [minterm](../Hardware_Manual_guide/node011D.html)  of AC),
                                               _
yielding our classic cookie-cut function of AB+AC, or $CA.
   ...
   function and preload the  [data register](../Hardware_Manual_guide/node011B.html#line31) .  Disabling a channel simply
   ...
   channel's  [data register](../Hardware_Manual_guide/node011B.html#line31) .
...
the same.
   ...
   Warning:
   --------
   Be sure to load the blitter immediate  [data registers](../Hardware_Manual_guide/node011B.html#line31)  only after
   setting the shift count in  [BLTCON0/BLTCON1](../Hardware_Manual_guide/node001A.html) , as loading the
```c
    [data registers](../Hardware_Manual_guide/node011B.html#line31)  first will lead to unpredictable results. For instance,
```
   ...
   "1<<4", not "1<<2". The act of loading one of the  [data registers](../Hardware_Manual_guide/node011B.html#line31) 
