...
Bus Error (/BERR)
   ...
   it, any device that drives /BERR must drive with an  [open collector](../Hardware_Manual_guide/node02BE.html)  or
...
System Reset (/RST, /BUSRST) := ( [/RESET, /IORST](../Hardware_Manual_guide/node02B1.html#line40) ) for Zorro III
   ...
   this reset capability, and is driven only by an  [open collector](../Hardware_Manual_guide/node02BE.html)  or
   ...
   expansion devices are required to reset their  [autoconfiguration](../Hardware_Manual_guide/node02C7.html)  logic
...
System Halt (/HLT)
   ...
   driven by a PIC with an  [open-collector](../Hardware_Manual_guide/node02BE.html)  or similar gate only. Its main
...
```c
    [open-collector](../Hardware_Manual_guide/node02BE.html)  or equivalent output type, and all are pulled high by
```
