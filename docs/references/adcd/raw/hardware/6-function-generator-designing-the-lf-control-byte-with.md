...
of the rows in the  [truth table](../Hardware_Manual_guide/node011C.html#line16)  corresponds to a "minterm", which is a
...
```
These correspond to  [BLTCON0](../Hardware_Manual_guide/node001A.html)  bit positions of 6, 4, 7, 3, and 2,
according to our  [truth table](../Hardware_Manual_guide/node011C.html#line16) , which we would then set, and clear the
rest.
...
To draw the car, we might use the A  [DMA channel](../Hardware_Manual_guide/node011B.html)  to fetch the car mask,
the B  [DMA channel](../Hardware_Manual_guide/node011B.html)  to fetch the actual car data, the C DMA channel to
fetch the background, and the D [DMA channel](../Hardware_Manual_guide/node011B.html)  to write out the new image.
...
that, whenever the car mask (fetched with  [DMA channel](../Hardware_Manual_guide/node011B.html)  A) had a bit set,
...
```c
          ...
          Selected      [BLTCON0](../Hardware_Manual_guide/node001A.html)             Selected      [BLTCON0](../Hardware_Manual_guide/node001A.html) 
