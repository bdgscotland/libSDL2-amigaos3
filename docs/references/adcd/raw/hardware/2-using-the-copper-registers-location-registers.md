...
counter and increments the program counter after each fetch. When a
 [jump address strobe](../Hardware_Manual_guide/node0051.html)  is written, the corresponding location register is
...
by another  [jump address strobe](../Hardware_Manual_guide/node0051.html) .

   About Copper restart.
   ---------------------
   At the start of each  [vertical blanking](../Hardware_Manual_guide/node0169.html)  interval, COP1LC is
   automatically used to start the program counter.  That is, no matter
   what the Copper is doing, when the end of  [vertical blanking](../Hardware_Manual_guide/node0169.html)  occurs,
