...
RDY-
   ...
   signal. When the motor is off this is used as a  [ID](../Hardware_Manual_guide/node01AB.html)  data line. See
   below.
...
```c
      second);
```
   *  System CPU executes a  [RESET](../Hardware_Manual_guide/node0171.html#line13)  instruction (DRESB- will go low for

```c
      approximately 17 microseconds);
```
   *   [Hard reset](../Hardware_Manual_guide/node0179.html)  from keyboard (lasts as long as keyboard reset is
