...
octave when the sample size is 16 bytes.  The " [Period](../Hardware_Manual_guide/node00DD.html) " column gives the
period count you enter into the  [period register](../Hardware_Manual_guide/node00DF.html#line34) . The length register
 [AUDxLEN](../Hardware_Manual_guide/node00DB.html#line26)  should be set to 8  (16 bytes = 8 words). The sample should
...
The table above shows the  [period values](../Hardware_Manual_guide/node00DF.html)  to use with a 16 byte sample to
...
When you double the  [period](../Hardware_Manual_guide/node00DD.html) , the time between each sample is doubled so
...
Thus, if you play a C with a  [period value](../Hardware_Manual_guide/node00DF.html)  of 214, then playing the same
sample with a  [period value](../Hardware_Manual_guide/node00DD.html)  of 428 will play a C in the next lower octave.
...
of a note.  To avoid  [aliasing distortion](../Hardware_Manual_guide/node00ED.html)  with these samples you should
use  [period values](../Hardware_Manual_guide/node00DF.html)  in the range 124-256 only.   [Periods](../Hardware_Manual_guide/node00DD.html)  from 124-256
correspond to playback rates in the range 14-28K samples per second which
makes the most effective use of the Amiga's 7 KHz  [cut-off filter](../Hardware_Manual_guide/node00EE.html)  to
...
to adjust the  [period value](../Hardware_Manual_guide/node00DD.html)  over its full range 124-65536.  This is
easier for the programmer but can produce undesirable high-frequency noise
in the resulting tone.  Read the section called  [Aliasing Distortion](../Hardware_Manual_guide/node00ED.html)  for
more about this.

The values in Table 5-7 were generated using the formula shown below.  To
calculate the tone generated with a given sample size and  [period](../Hardware_Manual_guide/node00DD.html)  use:
...
values were derived using the formula above.  Notice that in each octave
 [period values](../Hardware_Manual_guide/node00DF.html)  are the same but the sample size is halved.  The samples
...
```c
        ...
        ------  ------   ----   --------- -----------  ----------
         ...
         135     133      G#     103.82     103.57      104.17
               ...
               Sample size = 256 bytes,  [AUDxLEN](../Hardware_Manual_guide/node00DB.html#line26)  = 128
         ...
         135     133      G#     207.65     207.15      208.35
               ...
               Sample size = 128 bytes,  [AUDxLEN](../Hardware_Manual_guide/node00DB.html#line26)  = 64
         ...
         135     133      G#     415.30     414.30      416.70
                ...
                Sample size = 64 bytes,  [AUDxLEN](../Hardware_Manual_guide/node00DB.html#line26)  = 32
         ...
         135     133      G#     830.61     828.60      833.39
                ...
                Sample size = 32 bytes,  [AUDxLEN](../Hardware_Manual_guide/node00DB.html#line26)  = 16
         ...
         135     133      G#     1661.2    1657.2      1666.8
                ...
                Sample size = 16 bytes,  [AUDxLEN](../Hardware_Manual_guide/node00DB.html#line26)  = 8
