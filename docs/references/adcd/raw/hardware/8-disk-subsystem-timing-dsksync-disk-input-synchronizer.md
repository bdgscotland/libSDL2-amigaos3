...
highly useful when reading disks. If the WORDSYNC bit is enabled in
 [ADKCON](../Hardware_Manual_guide/node0194.html) , no data is transferred until a word is found in the input stream
...
In addition, the DSKSYNC bit in  [INTREQ](../Hardware_Manual_guide/node0037.html)  is set when the input stream
matches the DSKSYNC register. The DSKSYNC bit in  [INTREQ](../Hardware_Manual_guide/node0037.html)  is independent
