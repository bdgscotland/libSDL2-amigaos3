...
one color register, which is  [COLOR00](../Hardware_Manual_guide/node0067.html#line26) , the color of the background. You
treat the entire screen as a modification of that original color,
according to the scheme above.

Bit 11 of register  [BPLCON0](../Hardware_Manual_guide/node0092.html)  selects hold-and-modify mode. The following
bits in  [BPLCON0](../Hardware_Manual_guide/node0092.html)  must be set for hold-and-modify mode to be active:

   *  Bit HOMOD, bit 11, is 1.
   *  Bit  [DBLPF](../Hardware_Manual_guide/node007C.html) , bit 10, is 0 (single-playfield mode specified).
   *  Bit  [HIRES](../Hardware_Manual_guide/node0069.html#line7) , bit 15, is 0 (low resolution mode specified).
   *  Bits  [BPU2, BPU1, and BPU0](../Hardware_Manual_guide/node0068.html#line4)  - bits 14, 13, and 12, are 101 or 110

```c
                                    (five or six bitplanes active).

     [HAM_playfield.asm](../Hardware_Manual_guide/node02D1.html) 
```
