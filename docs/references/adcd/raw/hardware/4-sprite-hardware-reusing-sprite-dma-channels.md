...
Each of the eight sprite  [DMA channels](../Hardware_Manual_guide/node00BF.html)  can produce more than one
...
because you have  [attached](../Hardware_Manual_guide/node00C6.html)  some of the sprites to produce more colors or
larger objects or  [overlapped](../Hardware_Manual_guide/node00C5.html)  some to produce more complex images. You
can reuse each sprite  [DMA channel](../Hardware_Manual_guide/node00BF.html)  several times within the same display
...
In single-sprite usage, two all-zero words are placed at the end of the
 [data structure](../Hardware_Manual_guide/node00B9.html)  to stop the DMA channel from retrieving any more data for
...
you replace this pair of zero words with another complete sprite
 [data structure](../Hardware_Manual_guide/node00B9.html) , which describes the reuse of the DMA channel at a
position lower on the screen than the first use. You place the two
all-zero words at the end of the  [data structure](../Hardware_Manual_guide/node00B9.html)  that contains the
information for all usages of the DMA channel. For example, Figure 4-10
shows the  [data structure](../Hardware_Manual_guide/node00B9.html)  that describes the picture above.
...
needs the time during the blank line to fetch the  [control word](../Hardware_Manual_guide/node00B9.html#line24) 
...
data list is shown here. However, the sprite looks best with the
 [color registers](../Hardware_Manual_guide/node00D0.html)  set as shown in the example.
