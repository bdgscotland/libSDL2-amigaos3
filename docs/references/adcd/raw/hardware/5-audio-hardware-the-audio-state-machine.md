...
transitions. As long as the  [interrupt](../Hardware_Manual_guide/node016B.html)  is cleared by the processor in
time, the machine remains in the main loop. Otherwise, it enters the idle
state.  [Interrupts](../Hardware_Manual_guide/node016B.html)  are generated on every word transition (011->010).
...
counter is reloaded and an  [interrupt](../Hardware_Manual_guide/node016B.html)  request goes out soon after the
...
be repeated. If the  [filter](../Hardware_Manual_guide/node00EE.html)  on the Amiga is bypassed and the  [volume](../Hardware_Manual_guide/node00DC.html)  is
...
local signals.
   ...
   AUDxON      DMA on "x" indicates channel number (signal from  [DMACON](../Hardware_Manual_guide/node0170.html) ).

   AUDxIP       [Audio interrupt](../Hardware_Manual_guide/node016B.html)  pending (input to channel from interrupt

```c
               circuitry).
```
   AUDxIR       [Audio interrupt](../Hardware_Manual_guide/node016B.html)  request (output from channel to interrupt
...
```
   ...
   [AUDxDAT](../Hardware_Manual_guide/node0013.html)     Audio data load signal. Loads 16 bits of data to audio
...
```c
               by processor with  [AUDxPER](../Hardware_Manual_guide/node00DF.html#line34)  (can also be written by attach
               mode).
```
...
```
   ...
   see the  [ECS register map](../Hardware_Manual_guide/node00A1.html)  in Appendix C.
