...
In the following example, the Copper lists have been made to loop. The
 [COP1LC and COP2LC](../Hardware_Manual_guide/node0050.html)  values are either set via the CPU or in the Copper
...
the Copper to the next line before the next  [WAIT](../Hardware_Manual_guide/node004B.html) . The loop is executed
by writing to the  [COPJMP1](../Hardware_Manual_guide/node0051.html)  register. This causes the Copper to jump to
the address that was initialized in  [COP1LC](../Hardware_Manual_guide/node0050.html) .
...
1xxx1111). This is true for both the vertical and the horizontal  [WAIT](../Hardware_Manual_guide/node004B.html) 
instructions. To cause the second loop, write to the  [COPJMP2](../Hardware_Manual_guide/node0051.html)  register.
The list is put into an infinite wait when VP >= 255 so that it will end
before the vertical blank. At the end of the  [vertical blanking](../Hardware_Manual_guide/node0169.html)  period
 [COP1LC](../Hardware_Manual_guide/node0050.html)  is written to by the operating system, causing the first loop to
start up again.

```c
    [COP1LC](../Hardware_Manual_guide/node0050.html)  is written at the end of  [vertical blanking](../Hardware_Manual_guide/node0169.html) .
```
   ------------------------------------------------------
   The  [COP1LC](../Hardware_Manual_guide/node0050.html)  register is written at the end of the  [vertical blanking](../Hardware_Manual_guide/node0169.html) 
   period by a graphics interrupt handler which is in the vertical blank
   interrupt server chain.  As long as this server is intact,  [COP1LC](../Hardware_Manual_guide/node0050.html) 
   will be correctly strobed at the end of each vertical blank.

;
;  This is the data for the Copper list.
;
;  It is assumed that COPPERL1 is loaded into  [COP1LC](../Hardware_Manual_guide/node0050.html)  and
;  that COPPERL2 is loaded into  [COP2LC](../Hardware_Manual_guide/node0050.html)  by some other code.
;
COPPERL1:
