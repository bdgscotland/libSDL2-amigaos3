...
A light pen can be connected to one of the  [controller ports](../Hardware_Manual_guide/node017D.html) . On the
...
middle  [mouse button](../Hardware_Manual_guide/node0182.html) .

The principles of light pen operation are as follows:

   1. Just as the system exits  [vertical blank](../Hardware_Manual_guide/node0169.html) , the capture circuitry
...
```c
      save the current contents of the beam register,  [VPOSR](../Hardware_Manual_guide/node018A.html) . This
...
```
 [Reading the Light Pen Registers](../Hardware_Manual_guide/node018A.html) 
