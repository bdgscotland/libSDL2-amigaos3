...
```c
    ...
    by [AllocMiscResource](../Includes_and_Autodocs_2._guide/node054D.html).  The resource is made available
    for reuse.

    FreeMiscResource must be called from the same task that
    called [AllocMiscResource](../Includes_and_Autodocs_2._guide/node054D.html).  This function may not be called from
    interrupt code.
```
INPUTS
...
```c
    resources/misc.i, [AllocMiscResource()](../Includes_and_Autodocs_2._guide/node054D.html)
```
