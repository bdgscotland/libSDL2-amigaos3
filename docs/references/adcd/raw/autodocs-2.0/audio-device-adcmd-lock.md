...
```c
    ...
    Unlike setting the precedence (ADCMD_SETPREC, [ADCMD_ALLOCATE](../Includes_and_Autodocs_2._guide/node04A6.html) or
...
```c
    ...
    ioa_AllocKey- allocation key, must be set by (or copied from I/O block
                  set by) [OpenDevice](../Includes_and_Autodocs_2._guide/node04B8.html) function or [ADCMD_ALLOCATE](../Includes_and_Autodocs_2._guide/node04A6.html) command
```
OUTPUTS
