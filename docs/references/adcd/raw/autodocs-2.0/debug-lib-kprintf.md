...
```c
    ...
    exec [RawDoFmt()](../Includes_and_Autodocs_2._guide/node036C.html) call for the supported % formatting commands.
```
INPUTS
...
```c
    [exec.library/RawDoFmt](../Includes_and_Autodocs_2._guide/node036C.html), any C compiler's "printf" call.
```
