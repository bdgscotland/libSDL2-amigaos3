...
The [ObtainSemaphore()](../Includes_and_Autodocs_2._guide/node0362.html) function can be used to get an exclusive lock on a
semaphore.  If another task currently has an exclusive or [shared](../Libraries_Manual_guide/node02FB.html) lock(s)
...
```c
    Semaphore Nesting.
    ------------------
    [SignalSemaphore](../Libraries_Manual_guide/node02F7.html)s have nesting.  That is, if your task already
...
```
To get an exclusive lock on a [public](../Libraries_Manual_guide/node02F9.html) semaphore, the following code should
be used:
...
the semaphore is [public](../Libraries_Manual_guide/node02F9.html) and  might be removed by some other program).  If
...
[ObtainSemaphore()](../Includes_and_Autodocs_2._guide/node0362.html) function.
