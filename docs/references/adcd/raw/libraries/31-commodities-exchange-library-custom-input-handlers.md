...
```c
     [Figure 31-1: The Amiga Input Stream](../Libraries_Manual_guide/node05FA.html) 
```
...
```c
     [Figure 31-2: A Custom Input Handler](../Libraries_Manual_guide/node05FB.html) 
```
...
```c
     [Figure 31-3: The Commodities Network](../Libraries_Manual_guide/node05FC.html) 
```
...
event, it creates a [CxMessage](../Libraries_Manual_guide/node0403.html) (Commodities Exchange Message) corresponding
to the input event, and diverts the CxMessage through the network of
Commodities Exchange input handlers (Figure 31-3).

These handlers are made up of trees of different [CxObjects](../Libraries_Manual_guide/node0401.html) (Commodities
Exchange Objects), each of which performs a simple operation on the
[CxMessages](../Libraries_Manual_guide/node0403.html).  Any CxMessages that exit the network are returned to the
...
sometimes simply referred to as a commodity, uses the [CxObject](../Libraries_Manual_guide/node0401.html) primitives
to do things such as filter certain [CxMessages](../Libraries_Manual_guide/node0403.html), translate CxMessages,
