...
In the earlier sections of this chapter, the [basic environment](../Libraries_Manual_guide/node0009.html) of the
...
assembly language.
  ...
  * Check for [memory loss](../Libraries_Manual_guide/node0550.html).  Arrange your Workbench screen so that you
...
```c
    ...
    flush option requires the [Release 2](../Libraries_Manual_guide/node0015.html) version of the Avail command).
...
```
  * Use all of the program [debugging](../Libraries_Manual_guide/node055C.html) and stress tools that are available
...
```c
    exit and [program termination](../Libraries_Manual_guide/node02C0.html) due to error conditions. Anything that
    ...
    handles [Ctrl-C](../Libraries_Manual_guide/node0019.html#line7) properly.
```
...
```c
    memory location.  The only [absolute address](../Libraries_Manual_guide/node0013.html#line5) in the system is $0000
    ...
    includes the poking of [copper](../Libraries_Manual_guide/node031F.html) lists, memory lists, and library bases.
```
...
```c
    mutual exclusion ([locking](../Libraries_Manual_guide/node02C4.html)).  Remember that other tasks may be
    accessing the same structures.
```
  * The system does not monitor the size of a program's [stack](../Libraries_Manual_guide/node02BE.html#line12).  (Your

```c
    compiler may have an [option](../Libraries_Manual_guide/node0019.html#line32) to do this for you.)  Take care that your
...
```c
    ...
    delay.
```
  * [Tasks (and processes)](../Libraries_Manual_guide/node0287.html#line51) execute in 680x0 user mode.  [Supervisor mode](../Libraries_Manual_guide/node02CA.html) is
...
```c
    ...
    executed from a [process](../Libraries_Manual_guide/node0287.html#line51).  A task is not sufficient.  Most other ROM
    kernel functions may be executed from tasks.  Only a few may be
    executed from interrupts.
```
  * Never [disable interrupts](../Libraries_Manual_guide/node030E.html) or multitasking for long periods.  If you

```c
    use [Forbid()](../Libraries_Manual_guide/node02C2.html) or [Disable()](../Libraries_Manual_guide/node02C3.html), you should be aware that execution of any
    system function that performs the [Wait()](../Libraries_Manual_guide/node02F1.html#line31) function will temporarily
    ...
    and device I/O, including common stdio functions like [printf()](../Includes_and_Autodocs_2._guide/node0162.html).
```
  ...
  * All data for the [custom chips](../Libraries_Manual_guide/node0016.html) must reside in [Chip memory](../Libraries_Manual_guide/node0018.html) (type

```c
    [MEMF_CHIP](../Libraries_Manual_guide/node02A8.html#line11)).  This includes bitplanes, sound samples, trackdisk
    buffers, and images for sprites, bobs, pointers, and gadgets.  The
    [AllocMem()](../Libraries_Manual_guide/node02A7.html) call takes a flag for specifying the type of memory. A
    ...
    However, once [expansion memory](../Libraries_Manual_guide/node0018.html) has been added to an Amiga (type
    [MEMF_FAST](../Libraries_Manual_guide/node02A8.html#line20)), any memory allocations will be made in the expansion
    memory area by default.  Hence, a program can run correctly on an
    unexpanded Amiga which has only [Chip memory](../Libraries_Manual_guide/node0018.html) while [crashing](../Libraries_Manual_guide/node054A.html) on an
    ...
    Most compilers have [options](../Libraries_Manual_guide/node0019.html#line62) to mark specific data structures or
    object modules so that they will load into [Chip RAM](../Libraries_Manual_guide/node0018.html).  Some older
    compilers provide the Atom utility for marking object modules.  If
    this method is unacceptable, use the [AllocMem()](../Libraries_Manual_guide/node02A7.html) call to dynamically
    allocate Chip memory, and copy your data there.

    When making allocations that do not require [Chip memory](../Libraries_Manual_guide/node0018.html), do not
    explicitly ask for [Fast memory](../Libraries_Manual_guide/node0018.html).  Instead ask for memory type
    [MEMF_PUBLIC](../Libraries_Manual_guide/node02A8.html#line31) or 0L as appropriate.  If Fast memory is available, you
    will get it.
```
  * Never use software delay loops!  Under the [multitasking](../Libraries_Manual_guide/node000A.html) operating
...
```c
    ...
    the same on all models of the Amiga.  The AmigaDOS [Delay()](../Includes_and_Autodocs_2._guide/node0292.html) function
    or the graphics library [WaitTOF()](../Libraries_Manual_guide/node0390.html) function provide a simple interface
    for longer delays.  The [8520 I/O chips](../Hardware_Manual_guide/node012E.html) provide timers for developers
...
```c
      ...
      · Data structures to be accessed by the [custom chips](../Libraries_Manual_guide/node0016.html), public data
      ...
      · Dynamic allocation of structures with [AllocMem()](../Libraries_Manual_guide/node02A7.html) provides
        longword aligned memory of a specified type with optional
        initialization to zero, which is useful in the allocation of
        structures.
```
 [For 68010/68020/68030/68040 Compatibility](../Libraries_Manual_guide/node001B.html) 
 [Hardware Programming Guidelines](../Libraries_Manual_guide/node001C.html) 
 [Additional Assembler Development Guidelines](../Libraries_Manual_guide/node001D.html) 
