...
```c
    ...
    try to [Lock()](../Includes_and_Autodocs_2._guide/node02C6.html) or [Open()](../Includes_and_Autodocs_2._guide/node02D6.html) C:commandname to check for the command's
    existence.
```
...
```
  ...
  * By default, CON: now opens [SIMPLE_REFRESH](../Libraries_Manual_guide/node0125.html#line110) windows using the V36/V37

```c
    console character mapped mode.  Because of some differences between
    character mapped consoles, and [SMART_REFRESH](../Libraries_Manual_guide/node0125.html#line115) non-mapped consoles,
...
```c
    windows (both [SMART_REFRESH](../Libraries_Manual_guide/node0125.html#line115), and [SIMPLE_REFRESH](../Libraries_Manual_guide/node0125.html#line110) with character maps).
...
```c
    use [SMART_REFRESH](../Libraries_Manual_guide/node0125.html#line115) console windows, and use graphics.library calls
    mixed with console.device sequences to scroll, draw text, clear, etc.
    This is not supported in [SIMPLE_REFRESH](../Libraries_Manual_guide/node0125.html#line110) windows with character maps,
...
```
  * Note that [SIMPLE_REFRESH](../Libraries_Manual_guide/node0125.html#line110) character mapped consoles are immediately

```c
    redrawn with the global background color when changed--this is not
    possible with [SMART_REFRESH](../Libraries_Manual_guide/node0125.html#line115) windows.
```
