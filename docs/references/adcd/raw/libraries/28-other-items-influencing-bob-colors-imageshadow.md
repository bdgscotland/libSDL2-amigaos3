...
The [ImageShadow](../Libraries_Manual_guide/node0398.html#line5) member is a pointer to the shadow mask of a Bob. A shadow
mask is the logical or of all bitplanes of a Bob image. The system uses
the shadow mask in conjunction with [PlaneOnOff](../Libraries_Manual_guide/node03A1.html), discussed below, for color
...
Space for the [ImageShadow](../Libraries_Manual_guide/node0398.html#line5) must be provided and this pointer initialized to
...
```c
    Note:
    -----
    The [ImageShadow](../Libraries_Manual_guide/node0398.html#line5) memory must be allocated from Chip memory
    ([MEMF_CHIP](../Libraries_Manual_guide/node02A8.html#line11)).
```
