...
drivers are [MakeDosNode()](../Includes_and_Autodocs_2._guide/node03B8.html) and [AddDosNode()](../Includes_and_Autodocs_2._guide/node03B0.html).  These functions allow a
driver to create and add a DOS device node (for example DH0:) to the
system.   A new function, [AddBootNode()](../Includes_and_Autodocs_2._guide/node03AE.html), is also available in Release 2
(V36 and later versions of the OS) that can be used to add an [autobooting](../Libraries_Manual_guide/node041B.html)
DOS device node.

[MakeDosNode()](../Includes_and_Autodocs_2._guide/node03B8.html) requires an initialized structure of environment information
...
The parameter packet for [MakeDosNode()](../Includes_and_Autodocs_2._guide/node03B8.html) consists of four longwords followed
by a [DosEnvec](../Includes_and_Autodocs_2._guide/node0066.html#line28) structure:
...
```
After making a DOS device node, drivers (except for [autoboot](../Libraries_Manual_guide/node041B.html) drivers) use
[AddDosNode](../Includes_and_Autodocs_2._guide/node03B0.html)(deviceNode) to add their node to the system. Autoboot drivers
will instead use the new Release 2 expansion.library [AddBootNode()](../Includes_and_Autodocs_2._guide/node03AE.html)
function (if running under V36 or higher) or the Exec [Enqueue()](../Libraries_Manual_guide/node02E2.html) function
(if running under pre-V36) to add a [BootNode](../Libraries_Manual_guide/node041D.html#line22) to the
[ExpansionBase.eb_MountList](../Includes_and_Autodocs_2._guide/node00FA.html#line45).
