...
<libraries/[gadtools.h](../Includes_and_Autodocs_2._guide/node0106.html#line122)> as follows:
...
```c
    ...
    instead and point nm_Label at a valid [Image](../Libraries_Manual_guide/node01BA.html) structure.  The Image
    structure can contain just about any graphic image (see the chapter
    on "[Intuition Images, Line Drawing and Text](../Libraries_Manual_guide/node01B6.html)" for more on this).
...
```c
    ...
    [MenuItem](../Libraries_Manual_guide/node019A.html) structure.
```
nm_Flags

```c
    The nm_Flags field of the NewMenu structure corresponds roughly to
    the Flags field of the Intuition's lower-level [Menu](../Libraries_Manual_guide/node0199.html) and [MenuItem](../Libraries_Manual_guide/node019A.html)
    structures.
...
```c
    ...
    included in the position calculation.  See the "[Intuition Menus](../Libraries_Manual_guide/node018F.html)"
...
```c
    ...
    stored with the Intuition [Menu](../Libraries_Manual_guide/node0199.html) or [MenuItem](../Libraries_Manual_guide/node019A.html) structures that GadTools
    creates. Use the macros GTMENU_USERDATA(menu) and
    GTMENUITEM_USERDATA(menuitem) defined in <libraries/[gadtools.h](../Includes_and_Autodocs_2._guide/node0106.html#line173)> to
    ...
    program receives a [IDCMP_MENUPICK](../Libraries_Manual_guide/node01DE.html) message it may call the selected
