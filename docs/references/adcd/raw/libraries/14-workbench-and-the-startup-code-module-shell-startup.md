...
As with Workbench, standard startup code for the Shell sets up [SysBase](../Libraries_Manual_guide/node0013.html#line5),
the pointer to the Exec master library, and opens the DOS library setting
up [DOSBase](../Libraries_Manual_guide/node0290.html#line128).  C applications that are linked with standard startup code can
...
code:
  ...
  * If you are started as a Workbench process, you must [GetMsg()](../Libraries_Manual_guide/node02F2.html) the

```c
    [WBStartup](../Libraries_Manual_guide/node023E.html) message before using any functions in the DOS library.
```
  * You must turn off task switching (with [Forbid()](../Libraries_Manual_guide/node02D0.html#line10)) before replying the

```c
    [WBStartup](../Libraries_Manual_guide/node023E.html) message from Workbench.  This will prevent Workbench from
