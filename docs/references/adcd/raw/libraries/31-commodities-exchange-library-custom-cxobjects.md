...
Although the [CxObjects](../Libraries_Manual_guide/node0401.html) mentioned so far take care of most of the input
...
receives a [CxMessage](../Libraries_Manual_guide/node0403.html), it calls a function provided by the commodity.


```c
    customCxObj = CxObj  *CxCustom(LONG *customfunction(), LONG  cxmID);
```
A custom [CxObject](../Libraries_Manual_guide/node0401.html) is the only means by which a commodity can directly
modify input events as they pass through the Commodities network as
[CxMessages](../Libraries_Manual_guide/node0403.html).  For this reason, it is probably the most dangerous of the
CxObjects to use.
...
```c
    ...
    code passed to a custom [CxObject](../Libraries_Manual_guide/node0401.html) runs as part of the input.device
    ...
    [CxCustom()](../Includes_and_Autodocs_2._guide/node014B.html) should be very quick and very simple, with a minimum of
    stack usage.
```
Commodities Exchange calls a custom [CxObject](../Libraries_Manual_guide/node0401.html)'s function as follows:


```c
    void customfunction(CxMsg *cxm, CxObj *customcxobj);
```
where cxm is a pointer to a [CxMessage](../Libraries_Manual_guide/node0403.html) corresponding to a real input event,
and customcxobj is a pointer to the custom [CxObject](../Libraries_Manual_guide/node0401.html).  The custom function
can extract the pointer to the input event by calling [CxMsgData()](../Libraries_Manual_guide/node0403.html#line10).  Before
passing the CxMessage to the custom function, Commodities Exchange sets
the CxMessage's ID to the ID passed to [CxCustom()](../Includes_and_Autodocs_2._guide/node014B.html).

The following is an example of a custom [CxObject](../Libraries_Manual_guide/node0401.html) function that swaps
