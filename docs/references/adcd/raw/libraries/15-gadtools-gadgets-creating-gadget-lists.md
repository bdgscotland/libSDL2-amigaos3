...
In the discussion of [CreateGadget()](../Libraries_Manual_guide/node025A.html) presented earlier, the examples showed
...
used for three purposes.  First, when [CreateGadget()](../Libraries_Manual_guide/node025A.html) is called multiple
times, each new gadget is automatically linked to the previous gadget's
[NextGadget](../Libraries_Manual_guide/node0149.html#line21) field, thus creating a gadget list.  Second, if one of the
...
important to the application, certain calls to [CreateGadget()](../Libraries_Manual_guide/node025A.html) actually
...
failure after the last [CreateGadget()](../Libraries_Manual_guide/node025A.html) call and that is when the
...
```c
    ...
    gad = CreateGadget(..._KIND, gad, &ng2,
        ...
        TAG_END);
```
A major benefit of having a reusable [NewGadget](../Libraries_Manual_guide/node0259.html) structure is that often
many fields do not change and some fields change incrementally.  For
example, the application can set just the NewGadget's [ng_VisualInfo](../Libraries_Manual_guide/node0259.html#line49) and
[ng_TextAttr](../Libraries_Manual_guide/node0259.html#line34) only once and never have to modify them again even if the
