...
For images, the application must create one or more instances of the [Image](../Includes_and_Autodocs_2._guide/node00D4.html#line618)
structure.
...
```c
    ...
    The base position for images rendered with [DrawImage()](../Libraries_Manual_guide/node01BB.html) is taken from
...
```c
    PlanePick tells which planes of the target [BitMap](../Libraries_Manual_guide/node00F1.html#line4) are to receive
...
```c
    ...
    target [BitMap](../Libraries_Manual_guide/node00F1.html#line4) that receive no image data.  This field is a bit-wise
