...
the message port's queue are arranged in first-in-first-out ([FIFO](../Libraries_Manual_guide/node02EA.html#line15)) order
...
memory until task B returns the message to task A with the [ReplyMsg()](../Libraries_Manual_guide/node02F3.html)
function.
...
a pointer to a [Window](../Libraries_Manual_guide/node0121.html) structure, which contains a pointer to this message
...
for the task that opened the [Window](../Libraries_Manual_guide/node0121.html).  Because the most common message
...
can use the [WaitPort()](../Libraries_Manual_guide/node02F1.html#line9) function.  WaitPort() accepts a pointer to a
...
```
The Exec function [GetMsg()](../Libraries_Manual_guide/node02F2.html) is used to extract messages from the message
...
[ReplyMsg()](../Libraries_Manual_guide/node02F3.html).  Notice that the example keeps on trying to get messages from
...
Note that each task with a [Process](../Libraries_Manual_guide/node0287.html#line51) structure (sometimes referred to as a
process) has a special process message port, [Process.pr_MsgPort](../Includes_and_Autodocs_2._guide/node0078.html#line38).  This
message port is only for use by Workbench and the DOS library itself.  No
application should use this port for its own use!

 [Waiting on Message Ports and Signals at the Same Time](../Libraries_Manual_guide/node028D.html) 
