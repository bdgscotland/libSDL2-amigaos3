...
the Exec function [CachePreDMA()](../Libraries_Manual_guide/node02CE.html). If a DMA device is about to write to
memory, it should call CachePreDMA() before the write, do the DMA write,
and then call [CachePostDMA()](../Libraries_Manual_guide/node02CE.html),  which makes sure that the CPU uses the data
just written to memory.

An added advantage of using the [CachePreDMA()](../Libraries_Manual_guide/node02CE.html) and [CachePostDMA()](../Libraries_Manual_guide/node02CE.html) functions
...
the data cache after writing.  The V37 Exec function [CacheClearU()](../Includes_and_Autodocs_2._guide/node0339.html) takes
care of this.  Release 2 of the Amiga OS correctly flushes the caches as
needed after it does the [LoadSeg()](../Includes_and_Autodocs_2._guide/node02C5.html) of a program (LoadSeg() loads Amiga
...
CacheClearU() before the call to [CreateProc()](../Includes_and_Autodocs_2._guide/node028E.html).  In C that would be:
...
```c
    ...
    ***********************************************************************
```
Note that [CreateProc()](../Includes_and_Autodocs_2._guide/node028E.html) is not the only routine where CopyBack mode could
be a problem.  Any program code copied into memory for execution that is
not done via [LoadSeg()](../Includes_and_Autodocs_2._guide/node02C5.html) will need to call [CacheClearU()](../Includes_and_Autodocs_2._guide/node0339.html).  Many input device
