...
do this is listed in the next section, "[Advanced Screen Programming](../Libraries_Manual_guide/node00F7.html)".

Application programs that open [custom screens](../Libraries_Manual_guide/node00DF.html) may use the screen's display
...
[RastPort](../Libraries_Manual_guide/node00F1.html#line4), is not compatible with many Intuition constructs such as windows
and menus.
...
screen memory.  The [ShowTitle()](../Libraries_Manual_guide/node00FC.html#line10) function allows the screen's title bar
...
existing [public screens](../Libraries_Manual_guide/node00E6.html) do not have the same freedom to access the
screen's display memory as they do with [custom screens](../Libraries_Manual_guide/node00DF.html).  In general,
...
```c
    ...
    [public screen](../Libraries_Manual_guide/node00E6.html), is strictly illegal.  Accessing the underlying
    graphics structures of a screen may only be done on [custom screens](../Libraries_Manual_guide/node00DF.html)
    opened by the application itself.

    Do Not Perform Layers Operations Directly.
    ------------------------------------------
    While [layers](../Libraries_Manual_guide/node03E2.html) are not part of the graphics library, it is appropriate
    to mention them here.  Certain types of layers operations are not
    allowed with Intuition.  You may not, for example, call [SizeLayer()](../Libraries_Manual_guide/node03EF.html#line17)
    on a window (use [SizeWindow()](../Libraries_Manual_guide/node0129.html) instead). To access layers library
    features with screens, use Intuition windows!
```
A [custom screen](../Libraries_Manual_guide/node00DF.html) may be created to allow for modification of the screen's
...
registers.  See the Amiga [Hardware](../Hardware_Manual_guide/node0047.html) Reference Manual or the [graphics](../Libraries_Manual_guide/node0004.html)
