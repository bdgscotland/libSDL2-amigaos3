...
deadable, the qualifier [KCF_DEAD](../Includes_and_Autodocs_2._guide/node003A.html#line56) flag must be included in the entry for
...
areas as entries in the [LoKeyMap](../Libraries_Manual_guide/node0470.html) table.  The [LoKeyMapTypes](../Libraries_Manual_guide/node0471.html) table says that
there are four qualifiers for both: the requisite [KCF_DEAD](../Includes_and_Autodocs_2._guide/node003A.html#line56), as well as
[KCF_SHIFT](../Libraries_Manual_guide/node0472.html), [KCF_ALT](../Libraries_Manual_guide/node0472.html), and [KCF_CONTROL](../Libraries_Manual_guide/node0472.html).  The number of qualifiers determine
...
```c
    ...
    |                                                               |
    | The abbreviations A, C, S stand for [KCF_ALT](../Libraries_Manual_guide/node0472.html), [KCF_CONTROL](../Libraries_Manual_guide/node0472.html), and |
    | [KCF_SHIFT](../Libraries_Manual_guide/node0472.html), respectively.  Also note that the ordering is      |
    | reversed from that in the normal [KeyMap](../Libraries_Manual_guide/node046F.html) table.                |
    |_______________________________________________________________|
```
Because keys 20 and 25 each use three qualifier bits (not including
[KCF_DEAD](../Includes_and_Autodocs_2._guide/node003A.html#line56)), according to the table there must be 8 pairs of data, arranged
as shown. Had only [KCF_ALT](../Libraries_Manual_guide/node0472.html) been set, for instance, (not including
...
possible values: 0, [DPF_DEAD](../Includes_and_Autodocs_2._guide/node003A.html#line66) and [DPF_MOD](../Includes_and_Autodocs_2._guide/node003A.html#line66).  In the Amiga 1000 German keymap
...
If the flag byte is [DPF_DEAD](../Includes_and_Autodocs_2._guide/node003A.html#line66), then that particular key combination
...
an index into part(s) of the data area for ALL the deadable ([DPF_MOD](../Includes_and_Autodocs_2._guide/node003A.html#line66) set)
keys.

Before going further, an understanding of the structure of a descriptor
data area wherein [DPF_MOD](../Includes_and_Autodocs_2._guide/node003A.html#line66) is set for one (or more) of its members is
necessary. Referring to the example, we see that DPF_MOD is set for the
first and second pairs of bytes.  According to its [LoKeyMapTypes](../Libraries_Manual_guide/node0471.html) entry,
...
```c
    * The byte pair for the ALT position of the H key (key 25) is
      [DPF_DEAD](../Includes_and_Autodocs_2._guide/node003A.html#line66),3 so the index is 3.

    * The byte pair for the SHIFT position of the A key (key 20) is
      [DPF_MOD](../Includes_and_Autodocs_2._guide/node003A.html#line66), key20s-key20, so we refer to the table-of-bytes at
      key20s.
