...
external event.  Applications call the Exec [Wait()](../Includes_and_Autodocs_2._guide/node038A.html) function, directly or
...
can be used this way.  Task exceptions, described in the "[Exec Tasks](../Libraries_Manual_guide/node02C5.html)"
chapter, allow signals to act as a task-local interrupt.

The [Wait()](../Includes_and_Autodocs_2._guide/node038A.html) function specifies the set of signals that will wake up the
...
to awaken the task.  [Wait()](../Includes_and_Autodocs_2._guide/node038A.html) returns a mask indicating which signals
...
is a message at the message port.  See the "[Exec Messages and Ports](../Libraries_Manual_guide/node02F1.html)"
...
[Wait()s](../Includes_and_Autodocs_2._guide/node038A.html) for it.  In such cases the Wait() will be immediately satisfied,
and the task will not be put to sleep.

The [Wait()](../Includes_and_Autodocs_2._guide/node038A.html) function implicitly clears those signal bits that satisfied the
...
Be aware that using [Wait()](../Includes_and_Autodocs_2._guide/node038A.html) will break a [Forbid()](../Libraries_Manual_guide/node02C2.html#line9) or [Disable()](../Libraries_Manual_guide/node02C3.html#line6) state.
Wait() cannot be used in supervisor mode or within interrupts.

A task may [Wait()](../Includes_and_Autodocs_2._guide/node038A.html) for a combination of signal bits and will wake up when
...
check signals (a compiler for example) [SetSignal](../Includes_and_Autodocs_2._guide/node037E.html)(0,0) can be used to get a
...
```
[SetSignal()](../Includes_and_Autodocs_2._guide/node037E.html) can also be used to set or clear the state of the signals.
