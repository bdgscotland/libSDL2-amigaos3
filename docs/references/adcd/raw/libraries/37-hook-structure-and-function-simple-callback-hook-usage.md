...
A [Hook](../Libraries_Manual_guide/node04A3.html) function must accept the following three parameters in these
specific registers:
...
```c
      A0 - Pointer to the [Hook](../Libraries_Manual_guide/node04A3.html) structure.
...
need to look at environment specific data.  For example, [printf()](../Includes_and_Autodocs_2._guide/node0162.html) needs to
...
```
The next step is to initialize the [Hook](../Libraries_Manual_guide/node04A3.html) for use.  This basically means
that the fields of the Hook structure must be filled with appropriate
values.

The following simple function initializes a [Hook](../Libraries_Manual_guide/node04A3.html) structure.
...
```c
     [hooks1.c](../Libraries_Manual_guide/node05C5.html) 
```
