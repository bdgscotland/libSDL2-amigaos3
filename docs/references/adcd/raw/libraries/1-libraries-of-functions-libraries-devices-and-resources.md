...
[run-time](../Libraries_Manual_guide/node0010.html#line9) libraries.  The Amiga also has specialized function groups called
devices and resources that programmers use to perform basic I/O operations
or access low-level hardware.

Devices and resources are similar in concept to a shared [run-time](../Libraries_Manual_guide/node0010.html) library.
They are managed by [Exec](../Libraries_Manual_guide/node0013.html) and must be opened before they can be used.
...
Devices and resources are managed by [Exec](../Libraries_Manual_guide/node0013.html) just as libraries are.  For more
information on devices and resources, see the chapter on [Exec Device I/O](../Libraries_Manual_guide/node0299.html)
later in this book or refer to the Amiga ROM Kernel Reference Manual:
[Devices](../Devices_Manual_guide/node0000.html) for detailed descriptions of each device.
  _____________________________________________________________________
 |                                                                     |
 | What Every Amiga Programmer Should Know:                            |
 | ----------------------------------------                            |
 | The functions in the Amiga OS are accessed through shared [run-time](../Libraries_Manual_guide/node0010.html#line9)  |
 | libraries.  Libraries must be opened before their functions may be  |
 | used.  The system's master library, [Exec](../Libraries_Manual_guide/node0013.html), is always open.  The Exec |
 | function [OpenLibrary()](../Libraries_Manual_guide/node000D.html) is used to open all other libraries.         |
 |_____________________________________________________________________|
