...
applies to them. For more information on devices, see the "[Exec Device I/O](../Libraries_Manual_guide/node0299.html)"
section of this manual or the Amiga ROM Kernel Reference Manual:
[Devices](../Devices_Manual_guide/node0000.html).
...
Exec [OpenLibrary()](../Includes_and_Autodocs_2._guide/node0367.html) function:
...
```c
    ...
    34  Kickstart 1.3 -  This is almost the same as release 1.2 except it
                         has an Autobooting [expansion.library](../Libraries_Manual_guide/node0290.html#line128)
    ...
    37  Kickstart 2.04 - This is the general Release 2 revision for all
                         Amiga models.
```
The [OpenLibrary()](../Includes_and_Autodocs_2._guide/node0367.html) function looks for a library with a name that matches
...
available, [OpenLibrary()](../Includes_and_Autodocs_2._guide/node0367.html) returns NULL.  A version of zero in OpenLibrary()
...
When [OpenLibrary()](../Includes_and_Autodocs_2._guide/node0367.html) looks for a library, it first looks in memory.  If the
...
("[diskfont.library](../Libraries_Manual_guide/node0290.html#line128)"), OpenLibrary() will look for the library in the
directory that the LIBS: logical assign currently references.

If [OpenLibrary()](../Includes_and_Autodocs_2._guide/node0367.html) finds the library on disk, it takes care of loading it
...
After the library is loaded into memory and initialized, [OpenLibrary()](../Includes_and_Autodocs_2._guide/node0367.html) can
actually "open" the library.  It does this by calling the library's [Open](../Libraries_Manual_guide/node028F.html#line49)
...
If any step of [OpenLibrary()](../Includes_and_Autodocs_2._guide/node0367.html) fails, it returns a NULL value.  If
OpenLibrary() is successful, it returns the address of the library base.
The library base is the address of this library's [Library](../Libraries_Manual_guide/node0292.html) structure
(defined in <exec/[libraries.h](../Includes_and_Autodocs_2._guide/node009C.html#line32)>).  The Library structure immediately
follows the vector table in memory.

After an application is finished with a library, it must close it by
calling [CloseLibrary()](../Includes_and_Autodocs_2._guide/node0340.html):
...
was opened with [OpenLibrary()](../Includes_and_Autodocs_2._guide/node0367.html).

 [Library Vector Offsets (LVOs)](../Libraries_Manual_guide/node028F.html)    [Calling a Library Function](../Libraries_Manual_guide/node0290.html) 
