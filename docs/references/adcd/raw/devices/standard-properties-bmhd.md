...
encoding used, and other data necessary to understand the [BODY](../Devices_Manual_guide/node01B7.html) to
follow.
...
typedef UBYTE Compression;    /* Choice of compression algorithm
   ...
   is the byte run encoding described in [Appendix C](../Devices_Manual_guide/node01BD.html).  Do not compress
   across rows! */
#define cmpNone        0
...
The number of source bitplanes in the [BODY](../Devices_Manual_guide/node01B7.html) chunk is stored in nPlanes.  An
ILBM with a [CMAP](../Devices_Manual_guide/node01B0.html) but no BODY and nPlanes = 0 is the recommended way to
store a color map.

Note: Color numbers are color map index values formed by pixels in the
destination bitmap, which may be deeper than nPlanes if a [DEST](../Devices_Manual_guide/node01B2.html) chunk calls
...
in the [BODY](../Devices_Manual_guide/node01B7.html) chunk (see below).  The value mskHasTransparentColor indicates
...
byteRun1 is documented in [Appendix C](../Devices_Manual_guide/node01BD.html).
