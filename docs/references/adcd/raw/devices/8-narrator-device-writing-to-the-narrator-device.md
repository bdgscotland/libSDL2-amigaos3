...
You write to the narrator device by passing a [narrator_rb](../Devices_Manual_guide/node0115.html#line17) I/O request to
the device with [CMD_WRITE](../Includes_and_Autodocs_2._guide/node04C7.html) set in io_Command, the number of bytes to be
...
[narrator_rb](../Devices_Manual_guide/node0115.html#line17) struct shown in the [Device Interface](../Devices_Manual_guide/node0115.html) section.
...
The following describes the fields of the [narrator_rb](../Devices_Manual_guide/node0115.html#line17) structure:
...
```c
    ...
    with a "#" symbol.  See the [How to Write Phonetically for Narrator](../Devices_Manual_guide/node011B.html)
...
```c
    Pointer to a set of audio allocation maps.  See the "[Audio Device](../Devices_Manual_guide/node002A.html)"
    chapter for details.
```
nm_masks

```c
    Number of audio allocation maps.  See the "[Audio Device](../Devices_Manual_guide/node002A.html)" chapter
    for details.
```
volume
...
```c
    ...
    response to read requests.  See the [Reading from the Narrator Device](../Devices_Manual_guide/node011A.html)
...
```c
     [Speak_Narrator.c](../Devices_Manual_guide/node01AA.html) 
```
