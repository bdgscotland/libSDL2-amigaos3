...
[OpenDevice()](../Includes_and_Autodocs_2._guide/node04B8.html). Four primary steps are required to open the audio device:
...
```c
      [IOAudio](../Devices_Manual_guide/node001E.html#line8) using [AllocMem()](../Includes_and_Autodocs_2._guide/node0332.html).
```
...
just as if you had sent the [ADCMD_ALLOCATE](../Devices_Manual_guide/node002A.html) command (see below).  If the
...
OpenDevice().  For more on channel allocation and the  [ADCMD_ALLOCATE](../Devices_Manual_guide/node002A.html)
command, see the section on [Allocation and Arbitration](../Devices_Manual_guide/node0029.html) below.
