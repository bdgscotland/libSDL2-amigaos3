...
request to the device with [GPD_SETTRIGGER](../Includes_and_Autodocs_2._guide/node04BE.html) set in io_Command and the
...
```
A few points to keep in mind with the [GPD_SETTRIGGER](../Includes_and_Autodocs_2._guide/node04BE.html) command are:
...
```
   ...
   device read request ([GPD_READEVENT](../Devices_Manual_guide/node00B5.html)) or a system CMD_CLEAR command
   flushes the buffer.
