...
```c
         ct - 0 or pointer to the [IODRPReq](../Devices_Manual_guide/node0044.html#line23) structure passed to PCDumpRPort
...
```c
         ct - pointer to a [IODRPReq](../Devices_Manual_guide/node0044.html#line23) structure
...
[IODRPReq](../Devices_Manual_guide/node0044.html#line23) structure in ct should not be used except for those rare printers
...
double buffering to reduce the dump time which is why the [AllocMem()](../Includes_and_Autodocs_2._guide/node0332.html) calls
...
[Transfer()](../Devices_Manual_guide/node005D.html) function in the transfer.c module.  You should return
PDERR_NOERR after handling this case. The PCM-defines for the x parameter
from the file devices/[prtgfx.h](../Includes_and_Autodocs_2._guide/node003B.html) are PCMYELLOW, PCMMAGENTA, PCMCYAN and
PCMBLACK.
...
```c
         ct - error code
         x  - io_Special flag from the [IODRPReq](../Devices_Manual_guide/node0044.html#line23) structure
         y  - 0
```
...
do, though, you should check the [SPECIAL_NOFORMFEED](../Devices_Manual_guide/node0050.html#line10) bit in x. Don't issue
