...
The events in a [SMUS](../Devices_Manual_guide/node02AB.html) score are grouped into parallel "tracks".  Each track
...
```c
     ...
     per staff.  So one or two [SMUS](../Devices_Manual_guide/node02AB.html) tracks represents a
     CMN staff.
```
...
The musical score format FORM [SMUS](../Devices_Manual_guide/node02AB.html) is intended for simple loading and
playback of small scores that fit entirely in main memory.  So we chose to
store its tracks separately.

There can be up to 255 tracks in a FORM [SMUS](../Devices_Manual_guide/node02AB.html).  Each track is stored as a
[TRAK](../Devices_Manual_guide/node02B7.html) chunk.  The count of tracks (the number of TRAK chunks) is recorded
in the [SHDR](../Devices_Manual_guide/node02B3.html) chunk at the beginning of the FORM [SMUS](../Devices_Manual_guide/node02AB.html).  The TRAK chunks
