...
You write to the trackdisk device by passing an [IOExtTD](../Devices_Manual_guide/node00DE.html#line12) to the device with
[CMD_WRITE](../Includes_and_Autodocs_2._guide/node0522.html) set in io_Command, the number of bytes to be written set in
...
```
For writes using the enhanced write command ETD_WRITE, the [IOExtTD](../Devices_Manual_guide/node00DE.html#line12) is set
...
command ([see below](../Devices_Manual_guide/node00F0.html)).  If you wish to also write the sector label area, you
...
```c
    ...
    DoIO((struct IORequest *)DiskIO);
```
ETD_WRITE and [CMD_WRITE](../Includes_and_Autodocs_2._guide/node0522.html) obey all of the trackdisk device restrictions
