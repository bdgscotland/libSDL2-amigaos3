...
the playback parameters for the sampled waveforms in the [BODY](../Devices_Manual_guide/node02D0.html) chunk.  (See
...
A [FORM 8SVX](../Devices_Manual_guide/node02C9.html) holds waveform data for one or more octaves, each containing a
...
a power of two in order to play the [FORM 8SVX](../Devices_Manual_guide/node02C9.html) as a musical instrument in
...
pitch bends and vibrato.  A program that plays a [FORM 8SVX](../Devices_Manual_guide/node02C9.html) as a musical
...
chunk.  See "Data Chunk [BODY](../Devices_Manual_guide/node02D0.html)", below, for the layout of the octaves.

The field sCompression indicates the compression scheme, if any, that was
applied to the entire set of data samples stored in the [BODY](../Devices_Manual_guide/node02D0.html) chunk.  This
...
sCmpFibDelta is described in [Appendix C](../Devices_Manual_guide/node02D6.html).)  Note that the whole series of
...
[ATAK](../Devices_Manual_guide/node02CF.html) and [RLSE](../Devices_Manual_guide/node02CF.html), below).

Recording a one-shot sound.  To store a one-shot sound in a [FORM 8SVX](../Devices_Manual_guide/node02C9.html), set
...
store the compressed data in the [BODY](../Devices_Manual_guide/node02D0.html) chunk and set sCompression to the
...
Typically, creators of [FORM 8SVX](../Devices_Manual_guide/node02C9.html) record an acoustic instrument at just one
...
Playing a one-shot sound.  To play any [FORM 8SVX](../Devices_Manual_guide/node02C9.html) data as a one-shot sound,
...
envelope, too.  (See [ATAK](../Devices_Manual_guide/node02CF.html) and [RLSE](../Devices_Manual_guide/node02CF.html), below.) 

Playing a musical note.  To play a musical note using any [FORM 8SVX](../Devices_Manual_guide/node02C9.html), first
...
12-tone scale).  If the [FORM 8SVX](../Devices_Manual_guide/node02C9.html) doesn't have the right octave, you can
decimate or interpolate from the available data.

When it comes to musical instruments, [FORM 8SVX](../Devices_Manual_guide/node02C9.html) is geared for a simple
