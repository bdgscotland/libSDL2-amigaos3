...
any extra color registers. (See [Appendix E](../Devices_Manual_guide/node01C1.html) for a better way to write
colors)
...
```
Remember that every [chunk](../Devices_Manual_guide/node01DD.html) must be padded to an even length, so a color map
