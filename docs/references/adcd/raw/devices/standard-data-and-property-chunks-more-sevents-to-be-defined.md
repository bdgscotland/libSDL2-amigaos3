...
More [SEvent](../Devices_Manual_guide/node02B7.html#line29)s can be defined in the future.  The sID codes 133 through 143
...
The following [SEvent](../Devices_Manual_guide/node02B7.html#line29) types are under consideration and should not yet be
used.

Issue: A "change tempo" [SEvent](../Devices_Manual_guide/node02B7.html#line29) changes tempo during a score.  Changing the
tempo affects all tracks, not just the track containing the change tempo
event.

One possibility is a "scale tempo" [SEvent](../Devices_Manual_guide/node02B7.html#line29) SID_ScaleTempo that rescales the
global tempo:

	currentTempo := globalTempo * (data + 1) / 128

This can scale the global tempo (in the [SHDR](../Devices_Manual_guide/node02B3.html)) anywhere from x1/128 to x2
...
for coarse settings.  In this scheme, the [SHDR](../Devices_Manual_guide/node02B3.html)'s tempo is simply a
starting tempo.
...
implement tempo variations during the score.  But the "set tempo" [SEvent](../Devices_Manual_guide/node02B7.html#line29)
...
[SEvent](../Devices_Manual_guide/node02B7.html#line29)s.  This is a nice way to encode all the possible variations
...
event can reach as far as 512 [SEvent](../Devices_Manual_guide/node02B7.html#line29)s.  A second possibility is to call a
subtrack by index number, with an IFF chunk outside the [TRAK](../Devices_Manual_guide/node02B7.html) defining the
