...
```c
      ...
      then create it and add it via [AddResource()](../Includes_and_Autodocs_2._guide/node032A.html).  Under 2.0 the resource
...
```c
      to load it from the RDB area.  The list of [FileSysHeaderBlocks](../Devices_Manual_guide/node0079.html#line168)
      ...
      filesystem is found, then it is [LoadSeg()](../Includes_and_Autodocs_2._guide/node02C5.html)'ed from the "LSEG" blocks
...
```c
      [FileSysEntry](../Includes_and_Autodocs_2._guide/node011D.html#line32) structure (which is basically an environment vector for
