...
Instrument reference.  In [SSSP](../Devices_Manual_guide/node02AC.html#line8), each note event points to a "timbre
object" which supplies the "instrument" (the sound driver data) for that
note.  FORM [SMUS](../Devices_Manual_guide/node02AB.html) stores these pointers as a "current instrument setting"
for each track. It's just a run encoded version of the same information.
[SSSP reference](../Devices_Manual_guide/node02AC.html#line8) uses a symbol table to hold all the pointers to "timbre
object". [SMUS](../Devices_Manual_guide/node02AB.html) uses INS1 chunks for the same purpose.  They name the
score's instruments.
...
Instrument reference by name.  A reference from a [SMUS](../Devices_Manual_guide/node02AB.html) score to actual
...
Instrument reference by MIDI channel and preset.  A [SMUS](../Devices_Manual_guide/node02AB.html) score can also
...
Instrument reference via instrument register.  Each reference from a [SMUS](../Devices_Manual_guide/node02AB.html)
...
Each score has an array of [instrument registers](../Devices_Manual_guide/node02B0.html).  Each track has a
...
Track 2  | Set Inst 4 |  Note  |  Note  |  Note  |  Note  |  Note...  |
         ...
         +------------+--------+--------+--------+--------+-----------+
Locating instrument data by name.  "INS1" chunks in a [SMUS](../Devices_Manual_guide/node02AB.html) score name the
...
```
  2.Check any instrument [FORM](../Devices_Manual_guide/node01C7.html#line51)s embedded in the FORM [SMUS](../Devices_Manual_guide/node02AB.html).  (This is an
...
contents name vs. container name.  A musical instrument [FORM](../Devices_Manual_guide/node01C7.html#line51) should
