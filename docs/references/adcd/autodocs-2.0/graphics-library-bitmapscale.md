...
section: graphics-library-bitmapscale
functions: [ScalerDiv]
...
```c
    ...
    void BitMapScale(struct [BitScaleArgs](autodocs-2.0/includes-graphics-scale-h.md) *);
```
FUNCTION
...
```c
    ...
    o   No check is made to ensure destBitMap is big enough: use
        [ScalerDiv](autodocs-2.0/graphics-library-scalerdiv.md) to calculate a destination dimension.
```
BUGS
...
```c
    [ScalerDiv()](autodocs-2.0/graphics-library-scalerdiv.md)  [graphics/scale.h](autodocs-2.0/includes-graphics-scale-h.md)
```
