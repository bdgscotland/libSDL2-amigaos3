...
```c
    error = OpenDevice("console.device", unit, [IOStdReq](autodocs-2.0/includes-exec-io-h.md), flags )
    d0                  a0               d0    a1        d1
```
FUNCTION
...
```c
    ...
    fields in the [IOStdReq](autodocs-2.0/includes-exec-io-h.md) block that will be filled in: the
    ...
    io_Data field of the [IOStdReq](autodocs-2.0/includes-exec-io-h.md) block.  This initialization
    ...
    which is returned in the io_Device field of the [IOStdReq](autodocs-2.0/includes-exec-io-h.md)
    ...
    each character written with the [CMD_WRITE](autodocs-2.0/console-device-cmd-write.md) command.
...
IO REQUEST
```c
    io_Data         struct [Window](autodocs-2.0/includes-intuition-intuition-h.md) *window
...
```c
    ...
    [IOStdReq](autodocs-2.0/includes-exec-io-h.md) - a pointer to a standard request block
...
```c
    ...
    reasonable (e.g., no more than 1K characters per [CMD_WRITE](autodocs-2.0/console-device-cmd-write.md), and
...
```c
    [exec/io.h](autodocs-2.0/includes-exec-io-h.md), [intuition/intuition.h](autodocs-2.0/includes-intuition-intuition-h.md)
```
