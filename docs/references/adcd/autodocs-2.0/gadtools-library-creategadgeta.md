...
```c
    ...
    struct [Gadget](autodocs-2.0/includes-intuition-intuition-h.md) *CreateGadgetA(ULONG, struct [Gadget](autodocs-2.0/includes-intuition-intuition-h.md) *,
        struct [NewGadget](autodocs-2.0/includes-libraries-gadtools-h.md) *, struct [TagItem](autodocs-2.0/includes-utility-tagitem-h.md) *);

    gad = CreateGadget(kind, previous, newgad, firsttag, ...)

    struct [Gadget](autodocs-2.0/includes-intuition-intuition-h.md) *CreateGadget(ULONG, struct [Gadget](autodocs-2.0/includes-intuition-intuition-h.md) *,
        struct [NewGadget](autodocs-2.0/includes-libraries-gadtools-h.md) *, Tag, ...);
```
FUNCTION
...
```c
    ...
    gadget is created based on the supplied kind, [NewGadget](autodocs-2.0/includes-libraries-gadtools-h.md) structure,
    and tags.
```
INPUTS
...
```c
    ...
    newgad - a filled in [NewGadget](autodocs-2.0/includes-libraries-gadtools-h.md) structure describing the desired
...
```c
    ...
    GTST_EditHook (struct [Hook](autodocs-2.0/includes-utility-hooks-h.md) *) - (new for V37) [Hook](autodocs-2.0/includes-utility-hooks-h.md) to use as a custom
    ...
    GTLV_ShowSelected (struct [Gadget](autodocs-2.0/includes-intuition-intuition-h.md) *) - NULL to have the currently
    ...
    GTSL_LevelFormat (STRPTR) - C-Style formatting string for slider
        ...
        is processed using [exec/RawDoFmt()](autodocs-2.0/exec-library-rawdofmt.md), so refer to that function
        for details.
    ...
    GTSL_DispFunc ( LONG (*function)(struct [Gadget](autodocs-2.0/includes-intuition-intuition-h.md) *, WORD) ) - Function
...
```c
    ...
    [NewGadget](autodocs-2.0/includes-libraries-gadtools-h.md) structure must be set to valid VisualInfo and
    [TextAttr](autodocs-2.0/includes-graphics-text-h.md) pointers, or this function will fail.
...
```c
    [FreeGadgets()](autodocs-2.0/gadtools-library-freegadgets.md), GT_SetGadgetAttrs(), GetVisualInfo().
```
