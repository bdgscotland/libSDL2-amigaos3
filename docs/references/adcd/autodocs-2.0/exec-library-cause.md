...
SYNOPSIS
   Cause(interrupt)
         ...
         A1
   void Cause(struct [Interrupt](autodocs-2.0/includes-exec-interrupts-h.md) *);
...
```c
    When setting up the [Interrupt](autodocs-2.0/includes-exec-interrupts-h.md) structure, set the node type to
