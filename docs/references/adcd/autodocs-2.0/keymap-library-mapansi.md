...
```c
    ...
    LONG MapANSI( STRPTR, LONG, STRPTR, LONG, struct [KeyMap](autodocs-2.0/includes-devices-keymap-h.md) * );
```
FUNCTION
...
```c
    ...
    #include [<devices/inputevent.h>](autodocs-2.0/includes-devices-inputevent-h.md)

    #define STIMSIZE        3       /* two dead keys, one key */
    unsigned char rBuffer[STIMSIZE*2];
    ...
        KeymapBase = (struct [Library](autodocs-2.0/includes-exec-libraries-h.md) *) OpenLibrary("keymap.library", 0);
        ...
