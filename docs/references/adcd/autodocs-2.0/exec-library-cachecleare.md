...
section: exec-library-cachecleare
functions: [LoadSeg, SetFunction]
...
```c
    ...
    conflicts with the blitter or other custom chip DMA.  [Custom](autodocs-2.0/includes-hardware-custom-h.md) chip
    ...
    modify instructions, including [LoadSeg()](autodocs-2.0/dos-library-loadseg.md), [MakeLibrary()](autodocs-2.0/exec-library-makelibrary.md) and
    [SetFunction()](autodocs-2.0/exec-library-setfunction.md).
```
SEE ALSO

```c
    exec/execbase.i, [CacheControl](autodocs-2.0/exec-library-cachecontrol.md), [CacheClearU](autodocs-2.0/exec-library-cacheclearu.md)
```
