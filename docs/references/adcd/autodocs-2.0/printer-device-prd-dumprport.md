...
```c
    PRD_DUMPRPORT -- dump the specified [RastPort](autodocs-2.0/includes-graphics-rastport-h.md) to a graphics printer
```
FUNCTION

```c
    Print a rendition of the supplied [RastPort](autodocs-2.0/includes-graphics-rastport-h.md), using the supplied
    [ColorMap](autodocs-2.0/includes-graphics-view-h.md), position and scaling information, as specified in
...
```c
    ...
    io_RastPort     ptr to a [RastPort](autodocs-2.0/includes-graphics-rastport-h.md).
    io_ColorMap     ptr to a [ColorMap](autodocs-2.0/includes-graphics-view-h.md).
    io_Modes        the 'modes' flag from a [ViewPort](autodocs-2.0/includes-graphics-view-h.md) structure,
                    ...
                    ULONG ModeID = GetVPModeID(struct [ViewPort](autodocs-2.0/includes-graphics-view-h.md) *);
    ...
    io_SrcX         x offset into the [RastPort](autodocs-2.0/includes-graphics-rastport-h.md) to start printing from.
    io_SrcY         y offset into the [RastPort](autodocs-2.0/includes-graphics-rastport-h.md) to start printing from.
    io_SrcWidth     width of the [RastPort](autodocs-2.0/includes-graphics-rastport-h.md) to print (from io_SrcX).
    io_SrcHeight    height of the [RastPort](autodocs-2.0/includes-graphics-rastport-h.md) to print (from io_SrcY).
...
```c
    ...
    #include [<devices/prtgfx.h>](autodocs-2.0/includes-devices-prtgfx-h.md)
...
```
[Preferences](autodocs-2.0/includes-intuition-preferences-h.md)
-----------
...
PrintFlags         - CORRECT_RED, CORRECT_GREEN, CORRECT_BLUE,
                     ...
                     HALFTONE_DITHERING. FLOYD_DITHERING, ANTI_ALIAS,
                     GREY_SCALE2
...
```c
     struct [IORequest](autodocs-2.0/includes-exec-io-h.md) *ioreq;
...
a) struct [IOStdReq](autodocs-2.0/includes-exec-io-h.md)    a standard i/o request
b) struct [IODRPReq](autodocs-2.0/includes-devices-printer-h.md)    a dumprport i/o request
c) struct [IOPrtCmdReq](autodocs-2.0/includes-devices-printer-h.md) a printer command i/o request
...
```c
            ...
            =========================
```
In general densities which use more than one pass should only be used for
B&W shade dumps.  They can be used for Grey-Scale or Color Shade dumps BUT
the output may tend to look muddy or dark.  Also multiple pass Color dumps
tend to dirty or smear the ribbon (ie. yellow will get contaminated with
the other colors on the ribbon; you have been warned).
...
---------
1. Dot matrix b&w printer (text and graphics).
2. Drives all very old EpsonX (EX/FX/JX/LX/MX/RX, etc.) compatible
   printers.
3. Linefeeds # of vertical dots printed.
4. Density     XDPI    YDPI    XYDPI   Comments
