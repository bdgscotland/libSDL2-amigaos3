...
```c
    error = OpenDevice("narrator.device",  unit, [IORequest](autodocs-2.0/includes-exec-io-h.md), flags);
     D0                       A0            D0     A1         D1
```
FUNCTION
...
```c
    ...
    fields in the user's [IORequest](autodocs-2.0/includes-exec-io-h.md) block (see below).  If users wish
    ...
    the device node pointer in the [IORequest](autodocs-2.0/includes-exec-io-h.md) block and clears the
    delayed expunge bit.

    ***** NEW FOR V37 NARRATOR *****

    Several new fields in the [IORequest](autodocs-2.0/includes-exec-io-h.md) block have been added for V37
    ...
    [IORequest](autodocs-2.0/includes-exec-io-h.md) block.  Note that NDB_NEWIORB is set in the [IORequest](autodocs-2.0/includes-exec-io-h.md)
...
```c
    ...
    unit       - 0
    [IORequest](autodocs-2.0/includes-exec-io-h.md)  - Pointer to the user's [IORequest](autodocs-2.0/includes-exec-io-h.md) block
    flags      - 0
```
RESULTS

```c
    The narrator device will initialize the [IORequest](autodocs-2.0/includes-exec-io-h.md) block as follows
...
```c
    The include files contain the complete [IORequest](autodocs-2.0/includes-exec-io-h.md) block definition,
