...
section: graphics-library-qbsblit
functions: [QBlit]
...
```c
    ...
    void QBSBlit( struct [bltnode](autodocs-2.0/includes-hardware-blit-h.md) * );
```
FUNCTION

```c
    Call a user routine for use of the blitter, enqueued separately from
    the [QBlit](autodocs-2.0/graphics-library-qblit.md) queue.  Calls the user routine contained in the blit
...
```c
    [QBlit()](autodocs-2.0/graphics-library-qblit.md) [hardware/blit.h](autodocs-2.0/includes-hardware-blit-h.md)
```
