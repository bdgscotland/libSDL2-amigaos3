...
functions: [AutoRequest, BuildEasyRequestArgs, DisplayAlert, FreeSysRequest, ModifyIDCMP, Request, SysReqHandler, Wait]
...
```c
    ReqWindow = BuildSysRequest( [Window](autodocs-2.0/includes-intuition-intuition-h.md), BodyText, PosText, NegText,
    D0                           A0      A1        A2       A3
                       IDCMPFlags, Width, Height )
                       D0          D1     D2

    struct [Window](autodocs-2.0/includes-intuition-intuition-h.md) *BuildSysRequest( struct [Window](autodocs-2.0/includes-intuition-intuition-h.md) *, struct [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md) *,
            struct [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md) *, struct [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md) *, ULONG, WORD, WORD );
```
FUNCTION
...
```c
    ...
    [FreeSysRequest()](autodocs-2.0/intuition-library-freesysrequest.md) procedure to remove the requester and free up
    any allocated memory.

    See the autodoc for [SysReqHandler()](autodocs-2.0/intuition-library-sysreqhandler.md) for more information on the
    ...
    this function would try to call [DisplayAlert()](autodocs-2.0/intuition-library-displayalert.md) with the same
    ...
    pointer to an instance of the structure [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md).

    The BodyText argument should be used to describe the nature of
    the requester.  As usual with [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md) data, you may link several
    lines of text together, and the text may be placed in various
    locations in the requester.  This [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md) pointer will be stored
    ...
    the [DisplayAlert()](autodocs-2.0/intuition-library-displayalert.md) mechanism is used, this text will be rendered in
    ...
    New for V36, linked lists of [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md) are not correctly supported
    for gadget labels.
...
```c
    ...
    PosText = pointer to an [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md) structure
    NegText = pointer to an [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md) structure
    ...
    are some of the parameters of your [IntuiText](autodocs-2.0/includes-intuition-intuition-h.md), so that Intuition
...
```c
    ...
    this routine might have called [DisplayAlert()](autodocs-2.0/intuition-library-displayalert.md) before returning
    ...
    call [DisplayAlert()](autodocs-2.0/intuition-library-displayalert.md), or if it does, and there's not enough
    memory for the alert, the value of FALSE is returned.
```
BUGS

```c
    This procedure currently opens a window in the [Screen](autodocs-2.0/includes-intuition-screens-h.md) which
    ...
    [DisplayAlert()](autodocs-2.0/intuition-library-displayalert.md) is not called in version V36.

    It's almost impossible to make complete, correct account
    of different system fonts, window border dimensions, and
    screen resolution to get the layout of a System [Requester](autodocs-2.0/includes-intuition-intuition-h.md)
...
```c
    [FreeSysRequest()](autodocs-2.0/intuition-library-freesysrequest.md), [DisplayAlert()](autodocs-2.0/intuition-library-displayalert.md), [ModifyIDCMP()](autodocs-2.0/intuition-library-modifyidcmp.md), [exec.library/Wait()](autodocs-2.0/exec-library-wait.md),
    [Request()](autodocs-2.0/intuition-library-request.md), [AutoRequest()](autodocs-2.0/intuition-library-autorequest.md), EasyRequest(), [BuildEasyRequestArgs()](autodocs-2.0/intuition-library-buildeasyrequestargs.md)
```
