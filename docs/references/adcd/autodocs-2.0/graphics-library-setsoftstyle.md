...
section: graphics-library-setsoftstyle
...
```c
    ...
    ULONG SetSoftStyle(struct [RastPort](autodocs-2.0/includes-graphics-rastport-h.md) *, ULONG, ULONG);
```
FUNCTION
...
```c
    rp     - the [RastPort](autodocs-2.0/includes-graphics-rastport-h.md) from which the font and style
             are extracted.
    style  - the new font style to set, subject to enable.
    enable - those bits in style to be changed.  Any set bits here
             that would not be set as a result of [AskSoftStyle](autodocs-2.0/graphics-library-asksoftstyle.md) will
...
```c
    [AskSoftStyle()](autodocs-2.0/graphics-library-asksoftstyle.md)  [graphics/text.h](autodocs-2.0/includes-graphics-text-h.md)
```
