...
```c
    ...
    struct [Task](autodocs-2.0/includes-exec-tasks-h.md) *CreateTask(STRPTR,LONG,funcEntry,ULONG);
```
FUNCTION
...
```c
    ...
    [dos.library/CreateProc()](autodocs-2.0/dos-library-createproc.md) or the [dos.library/CreateNewProc()](autodocs-2.0/dos-library-createnewproc.md)
...
```c
    Under exec.library V37 or beyond, the [AddTask()](autodocs-2.0/exec-library-addtask.md) function used
    ...
    did not check for failure of [AddTask()](autodocs-2.0/exec-library-addtask.md) and thus might return a
...
```c
    [DeleteTask()](autodocs-2.0/amiga-lib-deletetask.md), [exec/FindTask()](autodocs-2.0/exec-library-findtask.md)
```
