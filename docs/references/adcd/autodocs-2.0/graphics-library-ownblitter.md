...
```c
    ...
    must do a [WaitBlit](autodocs-2.0/graphics-library-waitblit.md) before actually using the blitter registers.
...
```c
    [DisownBlitter()](autodocs-2.0/graphics-library-disownblitter.md) [WaitBlit()](autodocs-2.0/graphics-library-waitblit.md)
```
