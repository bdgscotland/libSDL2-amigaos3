...
```c
    ...
    void Deallocate(struct [MemHeader](autodocs-2.0/includes-exec-memory-h.md) *,APTR,ULONG);
```
FUNCTION
...
```c
    ...
    existing [MemHeader](autodocs-2.0/includes-exec-memory-h.md), however the extent pointers in the [MemHeader](autodocs-2.0/includes-exec-memory-h.md)
    will no longer be valid.

    If memoryBlock is not on a block boundary (MEM_BLOCKSIZE) then it
    will be rounded down in a manner compatible with [Allocate()](autodocs-2.0/exec-library-allocate.md).  Note
...
```c
    Allocate, [exec/memory.h](autodocs-2.0/includes-exec-memory-h.md)
```
