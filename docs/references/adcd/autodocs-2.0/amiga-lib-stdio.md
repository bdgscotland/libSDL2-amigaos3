...
```c
    ...
    fprintf - format data to file (see [printf()](autodocs-2.0/amiga-lib-printf.md))
    fputc   - put character to file
    fputs   - write string to file
    getchar - get a character from stdin
    [printf](autodocs-2.0/amiga-lib-printf.md)   - put format data to stdout (see exec.library/RawDoFmt)
...
```c
    ...
    To get a suitable AmigaDOS [FileHandle](autodocs-2.0/includes-dos-dosextens-h.md), the [dos.library/Open()](autodocs-2.0/dos-library-open.md) or
    [dos.library/Output()](autodocs-2.0/dos-library-output.md) functions must be used.

    All of the functions that write to stdout expect an appropriate
    [FileHandle](autodocs-2.0/includes-dos-dosextens-h.md) to have been set up ahead of time. Depending on
    ...
    From assembly:
            XDEF    _stdout
            DC.L    _stdout ;<- Place result of [dos.library/Output()](autodocs-2.0/dos-library-output.md) here.
```
