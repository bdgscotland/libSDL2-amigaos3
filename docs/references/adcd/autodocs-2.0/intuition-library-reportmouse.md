...
```c
    ReportMouse( Boolean, [Window](autodocs-2.0/includes-intuition-intuition-h.md) )
                 D0       A0        <-note

    VOID ReportMouse( BOOL, struct [Window](autodocs-2.0/includes-intuition-intuition-h.md) * );
```
SPECIAL NOTE
...
```c
    ...
    From assembler the interface has always been:
            Boolean in D0, [Window](autodocs-2.0/includes-intuition-intuition-h.md) in A0
    ...
    an atomic operation when faced with:
                    ...
                    Window->Flags &=~WFLG_REPORTMOUSE;
    or else bracket the operation between [Forbid()/Permit()](autodocs-2.0/exec-library-permit.md).
```
FUNCTION
...
```c
    ...
    The ReportMouse() function is first performed when [OpenWindow()](autodocs-2.0/intuition-library-openwindow.md)
...
```c
    [Window](autodocs-2.0/includes-intuition-intuition-h.md) = pointer to a [Window](autodocs-2.0/includes-intuition-intuition-h.md) structure associated with this request
