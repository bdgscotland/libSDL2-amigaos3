...
```c
    User [IORequest](autodocs-2.0/includes-exec-io-h.md) block (struct [narrator_rb](autodocs-2.0/includes-devices-narrator-h.md) as defined in .h file).
    The [OpenDevice](autodocs-2.0/narrator-device-opendevice.md) call will initialize the [IORequest](autodocs-2.0/includes-exec-io-h.md) block to a
    "standard male" voice.  If you want to change any parms, do so
    after the [OpenDevice](autodocs-2.0/narrator-device-opendevice.md) call and before the [DoIO](autodocs-2.0/exec-library-doio.md) (or SendIO/WaitIO).
    For a complete description of the [narrator_rb](autodocs-2.0/includes-devices-narrator-h.md) structure, see the
    narrator.h or .i include file.  Note that the [OpenDevice](autodocs-2.0/narrator-device-opendevice.md) call does
    not initialize all the fields needed by the narrator device.  The
    [IORequest](autodocs-2.0/includes-exec-io-h.md) fields which must be set by the user before issuing the
    write request are:
    ...
    requested by setting the mouths field of the [IORequest](autodocs-2.0/includes-exec-io-h.md) block to a
    ...
    of the [IORequest](autodocs-2.0/includes-exec-io-h.md) block.  Note that word and syllable sync only work
...
```c
    ...
    sets the io_Error field of the [IORequest](autodocs-2.0/includes-exec-io-h.md) block to an appropriate
