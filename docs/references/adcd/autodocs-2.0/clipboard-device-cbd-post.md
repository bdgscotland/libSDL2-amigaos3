...
```c
    ...
    post's io_ClipID with that returned by the [CBD_CURRENTREADID](autodocs-2.0/clipboard-device-cbd-currentreadid.md)
    ...
    [CBD_CURRENTWRITEID](autodocs-2.0/clipboard-device-cbd-currentwriteid.md) command.  If the current write io_ClipID is
