...
```c
    BltBitMap -- Move a rectangular region of bits in a [BitMap](autodocs-2.0/includes-graphics-gfx-h.md).
```
SYNOPSIS
...
```c
    ...
    ULONG BltBitMap(struct [BitMap](autodocs-2.0/includes-graphics-gfx-h.md) *, WORD, WORD, struct [BitMap](autodocs-2.0/includes-graphics-gfx-h.md) *,
...
```c
    ...
    area in a [BitMap](autodocs-2.0/includes-graphics-gfx-h.md) to another area, which can be on a different
    [BitMap](autodocs-2.0/includes-graphics-gfx-h.md).
    This blit is assumed to be friendly: no error conditions (e.g.
    a rectangle outside the [BitMap](autodocs-2.0/includes-graphics-gfx-h.md) bounds) are tested or reported.
```
INPUTS
...
```c
    SrcBitMap, DstBitMap - the BitMap(s) containing the
        ...
        - the planes copied from the source to the destination are
          ...
          than the minimum Depth of either [BitMap](autodocs-2.0/includes-graphics-gfx-h.md) and whose Mask
...
```c
    [ClipBlit()](autodocs-2.0/graphics-library-clipblit.md)  [graphics/gfx.h](autodocs-2.0/includes-graphics-gfx-h.md)  [hardware/blit.h](autodocs-2.0/includes-hardware-blit-h.md)
```
