...
```c
    ...
    struct [Interrupt](autodocs-2.0/includes-exec-interrupts-h.md) *SetIntVector(ULONG, struct [Interrupt](autodocs-2.0/includes-exec-interrupts-h.md) *);
```
FUNCTION
...
```c
    ...
    The IS_CODE and IS_DATA pointers of the [Interrupt](autodocs-2.0/includes-exec-interrupts-h.md) structure will
    be copied into a private place by Exec.  A pointer to the previously
    installed [Interrupt](autodocs-2.0/includes-exec-interrupts-h.md) structure is returned.
...
```c
    intNum - the Paula interrupt bit number (0..14).  Only non-chained
             interrupts should be set.  Use [AddIntServer()](autodocs-2.0/exec-library-addintserver.md) for server
             chains.
    interrupt - a pointer to an [Interrupt](autodocs-2.0/includes-exec-interrupts-h.md) structure containing the
...
```c
    [AddIntServer()](autodocs-2.0/exec-library-addintserver.md),exec/interrupts.i,hardware/intbits.i
```
