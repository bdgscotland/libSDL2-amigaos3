...
```c
    ...
    struct [DosList](autodocs-2.0/includes-dos-dosextens-h.md) *LockDosList(ULONG)
```
FUNCTION
...
```c
    ...
    for 1.3 compatibility, it also does a [Forbid()](autodocs-2.0/exec-library-forbid.md) - this will probably
    be removed at some future time.  The 1.3 [Forbid()](autodocs-2.0/exec-library-forbid.md) locking of this
    list had some race conditions.  The pointer returned by this is NOT
    an actual [DosList](autodocs-2.0/includes-dos-dosextens-h.md) pointer - you should use on of the other DosEntry
    calls to get actual pointers to [DosList](autodocs-2.0/includes-dos-dosextens-h.md) structures (such as
    [NextDosEntry()](autodocs-2.0/dos-library-nextdosentry.md)), passing the value returned by LockDosList()
    as the dlist value.
    ...
    and they're trying to send you a packet).  Use [AttemptLockDosList()](autodocs-2.0/dos-library-attemptlockdoslist.md)
    ...
    1.3/2.0 way:
            ...
            struct [DosList](autodocs-2.0/includes-dos-dosextens-h.md) *
            yourfindentry (struct [DosList](autodocs-2.0/includes-dos-dosextens-h.md) *dl, STRPTRname, type)
            {
...
```c
    [AttemptLockDosList()](autodocs-2.0/dos-library-attemptlockdoslist.md), [UnLockDosList()](autodocs-2.0/dos-library-unlockdoslist.md), [Forbid()](autodocs-2.0/exec-library-forbid.md), [NextDosEntry()](autodocs-2.0/dos-library-nextdosentry.md)
```
