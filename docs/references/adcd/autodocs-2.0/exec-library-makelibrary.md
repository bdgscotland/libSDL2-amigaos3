...
```c
    ...
    struct [Library](autodocs-2.0/includes-exec-libraries-h.md) *MakeLibrary
                          (APTR,struct [InitStruct](autodocs-2.0/exec-library-initstruct.md) *,APTR,ULONG,BPTR);
```
FUNCTION
...
```c
    ...
    init -  If non-NULL, an entry point that will be called before adding
            the library to the system.  Registers are as follows:
                    d0 = libAddr    ;Your [Library](autodocs-2.0/includes-exec-libraries-h.md) Address
                    a0 = segList    ;Your AmigaDOS segment list
                    a6 = [ExecBase](autodocs-2.0/includes-exec-execbase-h.md)  ;Address of exec.library
...
```c
    [InitStruct](autodocs-2.0/exec-library-initstruct.md), [InitResident](autodocs-2.0/exec-library-initresident.md), exec/initializers.i
```
