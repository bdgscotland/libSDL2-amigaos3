---
title: 19 / Using A Device / Synchronous Vs. Asynchronous Requests
manual: libraries
chapter: libraries
...
Synchronous requests use the [DoIO()](libraries/19-exec-device-i-o-using-a-device.md) function.  DoIO() will not return
...
Asynchronous requests use the [SendIO()](libraries/19-exec-device-i-o-using-a-device.md) and [BeginIO()](libraries/19-exec-device-i-o-using-a-device.md) functions.  Both
...
```c
    Do Not Touch!
    -------------
    When you use [SendIO()](libraries/19-exec-device-i-o-using-a-device.md) or [BeginIO()](libraries/19-exec-device-i-o-using-a-device.md), the I/O request you pass to
...
You can use the [WaitIO()](autodocs-2.0/exec-library-waitio.md), [Wait()](libraries/22-the-signal-system-waiting-for-a-signal.md) or [WaitPort()](libraries/24-messages-waiting-for-a-message.md) function to wait for the
return of the I/O request. It is important to note that all of the above
functions and also [DoIO()](libraries/19-exec-device-i-o-using-a-device.md) may Wait() on the message reply port's
[mp_SigBit](autodocs-2.0/includes-exec-ports-h.md).  For this reason, the task that created the port must be the
same task the waits for completion of the I/O.  There are three ways to
wait:


```c
      * [WaitIO()](autodocs-2.0/exec-library-waitio.md) not only waits for the return of the I/O request, it
      ...
      * [Wait()](libraries/22-the-signal-system-waiting-for-a-signal.md) waits for a signal to be sent to the message port.  It
        will awaken your task when the signal arrives, but you are
        responsible for all of the message handling.

      * [WaitPort()](libraries/24-messages-waiting-for-a-message.md) waits for the message port to be non-empty.  It
