# gadtools.library — Autodoc Reference
...
- **CreateContext** (V36) — create a place for GadTools context data.
- **CreateGadgetA** (V36) — allocate and initialize a gadtools gadget. (V36)
- **CreateMenusA** (V36) — allocate and fill out a menu structure. (V36)
- **DrawBevelBoxA** (V36) — draw a bevelled box. (V36)
- **FreeGadgets** (V36) — free a linked list of gadgets.
- **FreeMenus** (V36) — frees memory allocated by CreateMenusA().
- **FreeVisualInfo** (V36) — return any resources taken by GetVisualInfo.
- **GetVisualInfoA** (V36) — get information GadTools needs for visuals. (V36)
- **GT_BeginRefresh** (V36) — begin refreshing friendly to GadTools.
- **GT_EndRefresh** (V36) — end refreshing friendly to GadTools.
- **GT_FilterIMsg** (V36) — filter an IntuiMessage through GadTools.
- **GT_GetGadgetAttrsA** (V39) — request the attributes of a GadTools gadget.
- **GT_GetIMsg** (V36) — get an IntuiMessage, with GadTools processing.
- **GT_PostFilterIMsg** (V36) — return the unfiltered message after
- **GT_RefreshWindow** (V36) — refresh all GadTools gadgets in a window.
- **GT_ReplyIMsg** (V36) — reply a message obtained with GT_GetIMsg().
- **GT_SetGadgetAttrsA** (V36) — change the attributes of a GadTools gadget.
- **LayoutMenuItemsA** (V36) — position all the menu items. (V36)
- **LayoutMenusA** (V36) — position all the menus and menu items. (V36)

## Functions

### gadtools.library/CreateContext (V36)
...
glistptr - address of a pointer to a Gadget, which was previously
            set to NULL.  When all the gadget creation is done, you
            may use that pointer as your NewWindow.FirstGadget, or
...
### gadtools.library/CreateGadgetA (V36)

**allocate and initialize a gadtools gadget.CreateGadget -- varargs stub for CreateGadgetA().**
...
All kinds:
 GT_Underscore - Indicates the symbol that precedes the character in
     ...
     process the keys - it just displays the underscore).  For
 ...
 GA_TabCycle (BOOL) - Set to TRUE so that pressing <TAB> or
     <Shift-TAB> will activate the next or previous such gadget.
     (defaults to TRUE, unlike regular Intuition string gadgets
     which default to FALSE). (V37)
 ...
 GTIN_EditHook (struct Hook *) - Hook to use as a custom
     integer gadget edit hook (StringExtend->EditHook) for this
     gadget.  GadTools will allocate the StringExtend->WorkBuffer
     for you.  (defaults to NULL). (V37)
 ...
 GTLV_MakeVisible (WORD) - Number of an item that should be forced
     within the visible area of the listview by doing minimal
     scrolling.  This tag overrides GTLV_Top. (V39)
 ...
 GTLV_CallBack (struct Hook *) - Callback hook for various listview
     ...
     is called with:
         (struct Hook *, struct Node *, struct LVDrawMsg *)
     when its a PPC hook and with
         A0 - struct Hook *
         A2 - struct Node *
         A1 - struct LVDrawMsg *
     when its a 68k hook.
 ...
 GTMX_Active (UWORD) - The ordinal number (counting from zero) of
     the initially active choice of an mx gadget (defaults to 0).
     (V36)
 ...
 GTNM_Number (LONG) - A signed long integer to be displayed as a
     read-only number (defaults to 0). (V36)
 ...
 GTNM_Format (STRPTR) - C-Style formatting string to apply on the
     number before display. Be sure to use the 'l' (long) modifier.
     This string is processed using exec.library/RawDoFmt(), so
     refer to that function for details. (defaults to "%ld") (V39)
 ...
 GTPA_IndicatorHeight (UWORD) - The desired height of the
     current-color indicator, if you want one above the palette. (V36)
 ...
 GTSC_Arrows (UWORD) - Asks for arrows to be attached to the scroller.
     The value supplied will be taken as the width of each arrow
     button for a horizontal scroller, or the height of each button
     for a vertical scroller (the other dimension will match the
     whole scroller). (V36)
 ...
 GA_Immediate (BOOL) - If you want to hear each slider
     IDCMP_GADGETDOWN event (defaults to FALSE). (V36)
 ...
 GTSL_MaxLevelLen (UWORD) - Maximum length in characters of level
     string when rendered beside slider (defaults to 2). (V36)
 ...
 GTSL_LevelPlace - One of PLACETEXT_LEFT, PLACETEXT_RIGHT,
     ...
     indicator is to go relative to slider (default to
     PLACETEXT_LEFT). (V36)
...
**See also:** FreeGadgets(), GT_SetGadgetAttrs(), GetVisualInfo(),
 <libraries/gadtools.h>

---

### gadtools.library/CreateMenusA (V36)

**allocate and fill out a menu structure.CreateMenus -- varargs stub for CreateMenus().**
...
GTMN_FrontPen (UBYTE) - Pen number to be used for menu text.
 ...
 GTMN_SecondaryError (ULONG *) - Supply a pointer to a
     NULL-initialized ULONG to receive a descriptive error code.
     Possible values:
...
**See also:** LayoutMenusA(), FreeMenus(), gadtools.h/GTMENU_USERDATA(),
 gadtools.h/GTMENUITEM_USERDATA()

---

### gadtools.library/DrawBevelBoxA (V36)

**draw a bevelled box.DrawBevelBox -- varargs stub for DrawBevelBoxA().**
...
**See also:** GetVisualInfoA(), <libraries/gadtools.h>

---

### gadtools.library/FreeGadgets (V36)

**free a linked list of gadgets.**
...
**See also:** CreateGadgetA()

---

### gadtools.library/FreeMenus (V36)

**frees memory allocated by CreateMenusA().**
...
**See also:** CreateMenusA()

---

### gadtools.library/FreeVisualInfo (V36)

**return any resources taken by GetVisualInfo.**
...
**See also:** GetVisualInfoA()

---

### gadtools.library/GetVisualInfoA (V36)

**get information GadTools needs for visuals.GetVisualInfo -- varargs stub for GetVisualInfoA().**
...
**See also:** FreeVisualInfo(), intuition/LockPubScreen(),
 intuition/UnlockPubScreen()

---

### gadtools.library/GT_BeginRefresh (V36)

**begin refreshing friendly to GadTools.**
...
**See also:** intuition.library/BeginRefresh()

---

### gadtools.library/GT_EndRefresh (V36)

**end refreshing friendly to GadTools.**
...
**See also:** intuition.library/EndRefresh()

---

### gadtools.library/GT_FilterIMsg (V36)

**filter an IntuiMessage through GadTools.**
...
modimsg - a modified IntuiMessage, possibly with extra information
           ...
           the function should be sent back to Intuition via
           ReplyMsg()
...
**See also:** GT_GetIMsg(), GT_PostFilterIMsg()

---

### gadtools.library/GT_GetGadgetAttrsA (V39)

**request the attributes of a GadTools gadget.GT_GetGadgetAttrs -- varargs stub for GT_GetGadgetAttrsA().**
...
**See also:** GT_SetGadgetAttrs()

---

### gadtools.library/GT_GetIMsg (V36)

**get an IntuiMessage, with GadTools processing.**
...
**See also:** GT_ReplyIMsg(), GT_FilterIMsg()

---

### gadtools.library/GT_PostFilterIMsg (V36)

**return the unfiltered message after
                      GT_FilterIMsg() was called, and clean up.**
...
**See also:** GT_FilterIMsg()

---

### gadtools.library/GT_RefreshWindow (V36)

**refresh all GadTools gadgets in a window.**
...
**See also:** GT_BeginRefresh()

---

### gadtools.library/GT_ReplyIMsg (V36)

**reply a message obtained with GT_GetIMsg().**
...
### gadtools.library/GT_SetGadgetAttrsA (V36)

**change the attributes of a GadTools gadget.GT_SetGadgetAttrs -- varargs stub for GT_SetGadgetAttrsA().**
...
BUTTON_KIND:
 ...
 GTLV_Selected (UWORD) - Ordinal number of currently selected
     item. Starting with V39, you can provide ~0 to cause the
     currently selected item to be deselected. (V36)
 ...
 GTNM_Format (STRPTR) - C-Style formatting string to apply on the
     number before display. Be sure to use the 'l' (long) modifier.
     This string is processed using exec.library/RawDoFmt(), so
     refer to that function for details. (V39)
...
**See also:** GT_GetGadgetAttrs()

---

### gadtools.library/LayoutMenuItemsA (V36)

**position all the menu items.LayoutMenuItems -- varargs stub for LayoutMenuItemsA().**
...
Lays out all the menu items and sub-items according to
 the supplied visual information and tag parameters.  You would use
 this if you used CreateMenusA() to make a single menu-pane (with
 sub-items, if any), instead of a whole menu strip.
...
**See also:** CreateMenusA(), GetVisualInfoA()

---

### gadtools.library/LayoutMenusA (V36)

**position all the menus and menu items.LayoutMenus -- varargs stub for LayoutMenusA().**
...
GTMN_TextAttr (struct TextAttr *) - Text Attribute to use for
 ...
 GTMN_CharSet (ULONG) - Charset of the menu texts.  Should be
     specified with e.g. the cat_CodeSet value of the catalog
     your application opened.  When a non-zero value is specified,
     OpenDiskFont() will be used to open the GTMN_TextAttr font
     and/or screen font in the specified charset for the menu. (V51.4)
 GTMN_NewLookMenus (BOOL) - If you ask for WA_NewLookMenus for your
     window, you should ask for this tag as well.  This informs
     GadTools to use the appropriate checkmark, Amiga-key, and
     colors. (V39)
 GTMN_Checkmark (struct Image *) - If you are using a custom image
     for a checkmark (WA_Checkmark), also pass it to GadTools, so
     it can lay the menus out accordingly. (V39)
 GTMN_AmigaKey (struct Image *) - If you are using a custom image for
     the Amiga-key in menus (WA_AmigaKey), also pass it to GadTools,
     so it can lay the menus out accordingly. (V39)
 GTMN_FrontPen (ULONG) - This tag has existed for CreateMenus(),
     but now LayoutMenusA() has it too.  If a legitimate pen number
     is supplied, it is used for coloring the menu items (in
     preference to anything passed to CreateMenus()).
     If GTMN_NewLookMenus has been specified, this tag defaults to
     using the screen's BARDETAILPEN (until V40) or MENUTEXTPEN
     (starting from V46), else it defaults to "do nothing".
