...
#ifdef CCLESTE_AMIGA
static const char *verstag = "$VER: ccleste 1.4 (29.03.2026)";
long __stack = 32768;
#endif
...
#ifdef CCLESTE_NO_MIXER
/* amiport: stub out SDL_mixer types when building without audio */
...
static int Mix_OpenAudio(int a, int b, int c, int d) { (void)a;(void)b;(void)c;(void)d; return -1; }
static void Mix_CloseAudio(void) {}
static void Mix_Quit(void) {}
static Mix_Chunk* Mix_LoadWAV(const char* p) { (void)p; return NULL; }
static void Mix_FreeChunk(Mix_Chunk* c) { (void)c; }
static int Mix_PlayChannel(int ch, Mix_Chunk* c, int l) { (void)ch;(void)c;(void)l; return -1; }
static void Mix_HaltChannel(int ch) { (void)ch; }
static void Mix_Pause(int ch) { (void)ch; }
static void Mix_Resume(int ch) { (void)ch; }
static Mix_Music* Mix_LoadMUS(const char* p) { (void)p; return NULL; }
static void Mix_FreeMusic(Mix_Music* m) { (void)m; }
static int Mix_PlayMusic(Mix_Music* m, int l) { (void)m;(void)l; return -1; }
static int Mix_FadeInMusic(Mix_Music* m, int l, int f) { (void)m;(void)l;(void)f; return -1; }
static int Mix_FadeOutMusic(int f) { (void)f; return 0; }
static void Mix_HaltMusic(void) {}
static void Mix_PauseMusic(void) {}
static void Mix_ResumeMusic(void) {}
static const char* Mix_GetError(void) { return "no mixer"; }
#endif
...
#elif defined(CCLESTE_AMIGA)
static int scale = 2; /* 256x256 -- fits in RTG without scaling pressure */
#else
static int scale = 4;
#endif
...
static char* GetDataPath(char* path, int n, const char* fname) {
	...
	snprintf(path, n, "PROGDIR:data/%s", fname);
#else
#ifdef _WIN32
	char pathsep = '\\';
#else
...
static void ReadGamepadInput(Uint16* out_buttons) {
	...
	for (int i = 0; i < sizeof controller_mappings / sizeof *controller_mappings; i++) {
		...
		int pressed = SDL_GameControllerGetButton(controller, mapping.sdl_btn);
